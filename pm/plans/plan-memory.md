...
**FERNme** ([github.com/mirkofr/FERNme](https://github.com/mirkofr/FERNme); license to confirm) — an open-source "brain-like memory layer for AI agents" (surfaced via a Reddit post). A **fuzzy Hebbian graph** where memories strengthen, decay, and spread activation; the repo confirms **ACT-R decay + spreading activation + saturating Hebbian write with no LLM** (LLM-gating only for novel free text), SQLite/Postgres backends, controlled-vocabulary canonicalization, persistent user/project memory, forgetting + preference drift, mood/style memory, outcome-based learning, and user-owned editable memory. *Not yet evaluated for use.*
