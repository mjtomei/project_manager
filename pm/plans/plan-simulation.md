...
**Empirical grounding: capital markets as a measured, low-denial-cost instance.** The surplus → obligation translation is already operationalized and quantified in one domain — corporate capital markets, where the selection unit is the firm and the global-scale selection environment is the market. (Lead surfaced by a collaborating agent; the three citations are verified against source here, not taken on the agent's characterization — the inherited-verdict-laundering failure mode `CITATION_USE_AUDIT.md` exists to catch.)

- *Surplus = slack / idle cash.* A firm with sound operating cash flow but low payout is the firm-level analog of an H0 surplus-holder: retained cash whose deployment does not much change firm survival — a flat local landscape at the firm level.
- *The obligation concentrates on surplus-holders.* [Brav, Jiang, Partnoy & Thomas 2008, *J. Finance*](https://papers.ssrn.com/sol3/papers.cfm?abstract_id=948907) — 1,059 hedge-fund-activism events, 2001–2006 — find activists disproportionately target profitable, sound-cash-flow firms with low payout / low leverage / low dividend (the Jensen-1986 free-cash-flow agency profile), and post-intervention these firms *raise payout* and operating performance (~7% announcement abnormal return, no one-year reversal). The threat lands on the surplus, exactly as H0 predicts. *Scope: the targeting and payout-increase results are robust; the long-run net-value-creation interpretation is contested (Brav et al.'s own follow-ups and the "value creation or destruction?" line debate it). The mapping uses only the robust targeting/payout part.*
- *Competition itself disciplines slack.* [Nickell 1996, *JPE* 104:724–746](https://doi.org/10.1086/262040) — ~670 UK firms — finds more competition (more competitors / lower rents) raises total-factor-productivity *growth*, with reduced managerial slack ("compels effort") as the proposed mechanism. The selection environment raises the cost of idle surplus continuously, not only through discrete activist events. *Scope: the measured construct is TFP growth rate; "compels effort" is the interpreted mechanism, not a directly measured variable.*
- *The cost of not deploying is priced.* [Jenter & Kanaan 2015, *J. Finance*](https://onlinelibrary.wiley.com/doi/abs/10.1111/jofi.12282) and Kaplan & Minton 2012 quantify forced-CEO-turnover sensitivity to performance — an industry-performance fall from the 90th to the 10th percentile roughly *doubles* forced-turnover probability. *Scope: Jenter & Kanaan's headline is that this sensitivity partly reflects misattribution — CEOs punished for downturns beyond their control — so the disciplinary mechanism is real but noisy, not cleanly performance-targeted. Existence of the sensitivity supports H0; the misattribution is a refinement, and itself a testable signature of imperfect global-scale selection.*

**Why markets are the clean limit (an analytical claim, not the sources').** The market case is the *low-denial-cost* limit of H0: a firm has no evolved local evaluator — no dominance machinery, no felt local point of reference — to override, so surplus → obligation runs fast, near-completely, and measurably. That makes the firm the cleanest empirical isolation of the H0 mechanism, and predicts the translation should be *slower and more resisted* in high-denial-cost domains (individual humans, dynastic lineages — H1, H2) — which is exactly where the resistance shows up as generational dissipation and elite non-alignment. The market evidence therefore corroborates H0's *mechanism class* (surplus → obligation via global-scale selection is a real, measured dynamic) and serves as calibration for research question 3, but it is a different selection unit (firm, not lineage) and environment (capital market, not biological / cultural evolution) than H1 and H2, so it is not direct confirmation of the lineage-level instances. *(All citations abstract/secondary-source-verified; full-text audit pending.)*
