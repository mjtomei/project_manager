...
**Sign-off is also the rollout mechanism for process / acceptance-criteria updates.** Because it checks each step against the *current* standard, a process change does not require manual backfill: re-run sign-off on an existing PR — **including an already-merged one** — and it detects the gap against the new standard, records a note, and bounces the PR through the relevant steps under the updated prompting to bring it into compliance. Example: bug PRs implemented before the pre/post-fix-capture requirement have no captures; re-running sign-off flags the missing evidence and routes the PR back to regenerate it (re-run the repro/verify against the merged fix). This requires sign-off to be **re-runnable on existing/merged PRs** (single-PR mode, `pr-ff9b728`) and a bounce able to **re-open already-merged work**; the plan watcher can drive a batch re-sign-off to roll a new standard across a whole plan. The standard itself lives in the sign-off + step prompts, so updating the prompt *is* updating the acceptance bar.
