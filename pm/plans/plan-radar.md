...
> **Cross-plan note (2026-08-31, [[plan-jurisdiction]]):** in the "corporation without human limitations" framing, the radar is the **environment-contact organ** — the deliberately built sensing of what the world is doing that a human firm gets free through embodied employees, and one of the two directions of reality-contact the imagined corporation must construct (the other is outcome contact: the riscv-pareto campaign's eval oracle). The founding critique above reads directly in that frame: an engagement-optimized feed is a *broken sensory organ*, reporting what stimulates rather than what's there; the auditable per-metric triage is what makes this one trustworthy — the sensing-side analog of the campaign's refuse-self-reported-outcomes rule. The radar is also a second spring of jurisdiction's worthwhile-problem supply (demand from the environment, alongside demand from the front), and the campaign's roster-research and upstream-watching needs are radar-shaped tasks that become radar topics when this plan lands.
