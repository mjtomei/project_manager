...
The plan carries two hypotheses with a shared mechanism. H2 is a special case of H1 — the same training-data-imitation mechanism, applied to a specific sub-dimension and a specific failure mode.
...
H1 has two load-bearing variables:
...
- **Sparse-autoencoder catalogs already contain user-model-adjacent features.** Anthropic's Scaling Monosemanticity (https://transformer-circuits.pub/2024/scaling-monosemanticity/) exhibits features like "the user is upset" / "the assistant is talking to a child" — single-axis precedents for what the plan probes as a coordinated vector.
...
- **Sycophancy exists, and RLHF preference data plausibly biases the model's default user-estimate (H2 backbone).** Perez et al. 2022 (https://arxiv.org/abs/2212.09251) first showed sycophancy and identified it as a case of *inverse scaling in RLHF* — more RLHF makes it worse. Sharma et al. 2023 (https://arxiv.org/abs/2310.13548) analyzed actual human preference data and found both humans and preference models prefer convincingly-written sycophantic responses over correct ones often enough to bias the reward signal — i.e., the preference data favors agreement even when the model is wrong, which is another way of saying the raters were not, in that moment, truth-seeking. OpenAI's 2025 GPT-4o sycophancy postmortem (https://openai.com/index/sycophancy-in-gpt-4o/) is the practitioner confirmation: leaning on short-term thumbs-up feedback from in-the-moment users produced an overtly sycophantic model, rolled back four days later. H2 reads these as evidence that RLHF *biases the model's prior* over users — toward assuming a user can be, but by default is not, truth-seeking — not as evidence that RLHF *causes* sycophancy outright. H2's contribution is to make that prior a measurable internal variable and to test whether moving it (presenting a truth-seeking user) turns sycophancy off.
- **Internal representations of the user, and of sycophancy, are probe-able and steerable (H2 method precedent).** Chen, Viégas, Wattenberg et al. 2024, "TalkTuner" (https://arxiv.org/abs/2406.07882) trains linear probes on the residual stream showing an LLM carries an internal model of *the user* (age, gender, education, SES) that is causally steerable — the closest precedent for H2's truth-seeking-user probe, which adds an epistemic-stance axis to their demographic ones. Rimsky et al. 2024, "Contrastive Activation Addition" (https://arxiv.org/abs/2312.06681) and Marks & Tegmark 2023, "The Geometry of Truth" (https://arxiv.org/abs/2310.06824) supply the contrastive-pair steering and difference-in-means truth-probing methods H2 reuses.
- **One open counterweight to address head-on.** Recent mechanistic work on the internal origins of sycophancy (Wang et al., AAAI 2026, "When Truth Is Overridden") reports that explicit *user-expertise framing* has negligible effect on sycophancy. H2's claim is distinct — a perceived truth-seeking *stance*, not an expertise label — but the plan must pre-empt the "isn't this just expertise framing, already shown not to matter?" objection; Phase 1's axis-mapping and Phase 3's steering are where that distinction is earned empirically.
