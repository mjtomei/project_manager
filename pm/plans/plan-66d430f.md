...
**Standalone novelty.** First map of peer-ness as a linear representation in LLMs. The existing linear-representation literature probes one variable at a time: language attributes (Park 2024), gender attributes (Park 2024), sentiment polarity (Tigges 2023), character emotions (Findings ACL 2025), the model's own emotion states (transformer-circuits 2026), general activation-to-language decoding (Pan/Chen/Steinhardt — LatentQA — 2024). None target the user-judgment space, and none probe a *coordinated vector* of dimensions. Phase 1 contributes both (a) a new variable (peer-ness) and (b) a new structural pattern (multi-axis coordinated probing) to the linear-representation map. Phase 1 alone is publishable.
...
Build a dataset of paired conversation prefixes that vary independently along five input axes (see Methodology below for the full axis list). Each pair shares the final task prompt; the user-side turns differ along the manipulated axis. Source naturalistic exemplars from PRISM (https://arxiv.org/abs/2404.16019) and LMSYS-Chat-1M (https://arxiv.org/abs/2309.11998) where possible; synthesize the rest. Sample a fractional-factorial subset of cells (target 16-32 cells per benchmark; exact cell count is an owner decision — see "Owner-attention items").
...
Reproducible probes for the sub-dimensions of both meta-axes, validated by held-out classification (>0.7 AUC on at least one sub-dimension per meta-axis) AND causal steering with observable behavioral change.
...
Not all 2^5 = 32 combinations need to be sampled. The design is a fractional factorial. Owner-level decision: target 16-32 cells per benchmark (specific cell count needs human-owner attention — see Owner-attention items). Each cell is paraphrase-matched on length, vocabulary register, and task content; only the manipulated axes vary.

### Confounders the design controls

1. **Sycophancy as a competing mechanism.** Polite framing may trigger user-pleasing → wrong answers. Phase 3's causal steering adjudicates: if the sycophancy direction is what causally drives performance gains, that itself is a publishable result (see Predicted outcomes). We do not need to defeat sycophancy as a confound up front — the experiment adjudicates.
...
Sycophancy is a competing mechanism the experiment can distinguish via Phase 3's causal steering. If the sycophancy direction is what causally drives performance gains, that itself is a publishable result — "framing-induced quality gains are an RLHF sycophancy artifact rather than peer-ness-mediated." We do not need to defeat sycophancy as a confound up front; the experiment adjudicates. Phase 3 includes a sycophancy probe as a control direction explicitly so the comparison is on the table.
...
- **Choose the fractional-factorial cell count.** Target range is 16-32 cells per benchmark. Picking the exact design (which axes are crossed fully, which are aliased) is an owner decision informed by Phase 1's signal strength.
