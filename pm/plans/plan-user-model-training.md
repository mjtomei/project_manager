...
This is the *same principle* the user-model litreview already states (extension §2.3: the model should serve the user's *real* inclination toward a good outcome, and the failure is latching onto the *performed* or *self-perceived* channel) — applied one level up. There, the model infers real-vs-performed demand within an interaction; here, contributor filtering selects for humans whose *real* demand genuinely is truth-seeking, so the captured training signal is clean rather than validation-shaped at the source. One principle, two levels: real-over-performed at inference time, and real-over-performed at data-sourcing time, both requiring exogenous outcome-grounding to tell the two apart.
