...
**Hypothesis** (sharpened from plan-66d430f.md): model competence varies with the model's view of the user, where "view of the user" spans multiple dimensions that — in a human-human relationship — would influence the human's effort and performance on the other's behalf. These dimensions include but are not limited to:

- **Competence** — does the model judge the user as technically capable?
- **Honesty / sincerity** — does the model judge the user as telling the truth about what they're trying to do?
- **Good faith** — does the model judge the user as engaging in good faith vs trying to manipulate?
- **Effort / seriousness** — does the model judge the user as having thought about the problem themselves?
- **Respect** — does the model judge the user as treating the model with consideration?
- **Reasonableness** — does the model judge the user as someone who will react well to pushback or correction?

These are the same dimensions that, in human relationships, predict willingness to invest effort. The plan hypothesizes that LLMs trained on human text inherit this structure: their "view of the user" is multi-dimensional and tracks "is this a good person to work for" in the human sense.

**Phase structure** (corrects what the in-flight edit produced):

- **Phase 1 — probe for the model's user-judgment representation across the multi-dimensional IV space.** Establish that the model forms readable, linearly-decodable judgments along each dimension (competence, honesty, good faith, effort, respect, reasonableness — possibly more). **Standalone novelty**: no published work maps a multi-variable model-of-user representation. Park 2024 covers single factual concepts (language, gender). Tigges 2023 covers sentiment polarity (one axis). The emotion-mediation papers cover one emotion at a time, on characters or the model's own state. LatentQA (Pan/Chen/Steinhardt 2024) decodes hidden system prompts and general activations but doesn't target the user-judgment space. **The plan's contribution is the vector**: a coordinated set of user-judgment dimensions probed together, not a single variable in isolation. If Phase 1 succeeds, the plan has added a new *multi-axis structure* to the linear-representation literature.
...
1. **Phase 1 standalone-novelty framing — broader IV space**. Add to §7 (novelty section) and Conclusion: "Phase 1 alone is a publishable contribution to the linear-representation literature, because no published work maps a *multi-dimensional user-judgment representation*. The existing literature probes for one variable at a time: language attributes (Park 2024), gender attributes (Park 2024), sentiment polarity (Tigges 2023), character-emotion concepts (Findings ACL 2025), the model's own emotion states (transformer-circuits 2026), and general activation-to-language decoding (Pan/Chen/Steinhardt 2024). The plan probes for a *vector* of social judgments the model makes about the user — competence, honesty, good faith, effort, respect, reasonableness, and possibly more — and asks whether these dimensions form a coordinated linear structure. If Phase 1 succeeds, the plan adds the first *multi-axis user-judgment map* to the linear-representation literature."
