...
1. **Distinction-collapsing framing.** An entry treats a prior work as "near-twin" with the artifact, eliding a distinction the prior work itself preserves. Pattern: the artifact probes one dimension while the prior work probes a different one, and the source paper explicitly preserves the distinction — but the entry's framing collapses it into sameness, making the artifact's residual contribution look smaller than it actually is.
   ...
   *Example.* A source paper studies how a language model represents *user demographic attributes* (age, profession, etc.) via static profile prompts. The artifact studies how the model represents *user intent dimensions* (the user's current task goal) via dynamic task-tracking probes. An entry on the source, written in the artifact's context, calls it a "near-twin" of the artifact's contribution. The source's methodology section explicitly notes that intent representation is a separate question its setup doesn't address. The audit catches the collapse by reading the methodology rather than the abstract — the abstract's headline result is consistent with "near-twin" framing; the methodology section is where the distinction is preserved.
...
2. **Inherited-verdict laundering.** An entry takes a prior agent's characterization (e.g., a lit-search summary) at face value without independently reading the source. Symptoms: confident "largely pre-empted by X" claims that vanish when the figures and methodology in X are checked against the artifact's actual scope. The verdict reads as a verification but the verification never happened.
   ...
   *Example.* The review flags a paper in the artifact's missing-citations list as "largely pre-empting the artifact's main contribution," citing the paper's abstract. The audit reads the full paper and finds the headline result was reported on a different model class than the artifact targets (encoder-only transformers vs. the artifact's decoder-only setting), under an evaluation regime that doesn't transfer (perplexity, not task success). The "pre-emption" framing came from a lit-search agent that read the abstract in isolation; the source itself never makes that claim. The audit catches it by verifying the source's scope conditions against the artifact's setting before propagating the verdict.
...
3. **Mechanism misnaming.** An entry uses a plausible-sounding label for a source's mechanism that doesn't match what the source describes. Pattern: the entry says the source did *method-name-A* (which would land cleanly in the artifact's argument) when the source actually does *method-name-B* (which lands differently or not at all). The methodology section of the source explicitly contradicts the entry, but the entry never reads that far.
   ...
   *Example.* An entry summarizes a source's training method as "REINFORCE against a natural-text baseline" — a phrase that fits cleanly into the artifact's argument about reward shaping. The source's methodology section describes the method as a sibling-rationale-baselined REINFORCE with m-token lookahead: a similar-sounding but structurally different gradient estimator that produces a different signal in practice (and whose theoretical justification is different too). The audit catches the misnaming by reading the methodology section rather than relying on the abstract's compressed description.

4. **Scope conflation.** An entry generalizes a method's domain beyond what the source claims — applying a method developed for one setting as if it covers a broader setting. Pattern: the entry summarizes the headline result correctly but elides the scope condition that determines whether the result transfers.

   *Example.* A source paper's title and abstract describe "fuzzing for multi-agent LLM systems." An entry on the paper, written in an artifact whose argument is about *general-program* fuzzing, characterizes the source as "a fuzzing approach for general programs." The source's abstract explicitly scopes the contribution to multi-agent LLM input structures; transfer to general programs would require assumptions the source does not make. The audit catches the conflation by reading the abstract carefully — the scope is in the title itself.
