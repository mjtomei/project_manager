...
**Role under the new literature review flow:** with `LITERATURE_REVIEW_FLOW.md` in place, citation discipline (existence, faithfulness, alternative-perspective representation) is enforced by Phases 1–3 of that flow rather than by this protocol. This protocol's role narrows to **prose, structure, and accessibility critique** of the assembled synthesis — Block 2 (structure), Block 3 (accessibility, **load-bearing under Phase 5**), Block 4 (writing quality), and the substance-questions in Block 1 that are *not* about citations. Citation-related questions in Block 1 still apply when the protocol is run on artifacts produced outside the new flow.

**Block 3 (accessibility) remains load-bearing under Phase 5.** The new flow's citation-and-synthesis discipline guarantees the artifact's *content* is defensible against its sources, but accessibility is orthogonal — an artifact can be source-faithful and synthesis-clean and still unreadable to its stated audience. Block 3's job (undefined jargon, implicit prior-art dependencies, unmotivated framings, abstract claims without examples, names dropped without context, scale anchors on quantitative claims, "why should I care" checks) is what determines whether the assembled lit review meets its audience obligation. A Phase 5 pass that catches every prose tic but misses the accessibility failures has missed the artifact's core obligation — same as under the old flow.

**Net-cut and verbosity discipline (steps 10–11) is now in `LITERATURE_REVIEW_FLOW.md` Phase 5.** Steps 10–11 below remain the procedural source; the Phase 5 section restates the discipline as mandatory because the new flow inherits the same growth bias.
