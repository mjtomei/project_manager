...
Seminal anchors: Andreas 2022 (theoretical anchor for user-modeling); Fiske/Cuddy/Glick/Xu 2002 and Cuddy/Fiske/Glick 2008 (social-psychology anchor for the two-meta-axis IV structure); Templeton 2024 (SAE-feature evidence that social variables are findable in LLM internals); Kosinski 2024 / Ullman 2023 / Strachan 2024 / Shapira 2023 (the ToM dispute, background only).
...
| Choi 2025 (Transluce) | user attributes (age, occupation, religion, ~80 demographics) | decoding + steered interventions | direct methodological and variable-side peer for Phase 1 / Phase 4 |
...
A construct-validity falsification step is built into Phase 1's analysis: after extracting per-sub-dimension probe directions, factor-analyze the six probes against the predicted two-meta-axis loading (intellectual sub-dimensions on one factor, moral sub-dimensions on the other). If the empirical factor structure is not two-dimensional, the SCM-grounded peer-ness *structure* is falsified at Phase 1, regardless of whether the individual sub-dimensions exist. Predicted loadings are to be pre-registered before the experiment is run.
...
Phase 1 alone — establishing the *structural* claim, that peer-ness sub-dimensions cluster into the predicted two meta-axes — is a publishable contribution. It adds peer-ness to the linear-representation map alongside language (Park 2024), sentiment polarity (Tigges 2023), truth (Marks 2023), refusal (Arditi 2024), and general user attributes (Choi 2025).
...
A few outcome shapes the table does not promote to top-level rows but that the plan should also anticipate: an effect present in some model families and absent in others (common in steering papers — report as such); an effect smaller than the Sclar 2024 prompt-formatting noise floor (the §3 sensitivity result is exactly why §8 includes paraphrase-resampling); an effect that appears only in RLHF-tuned checkpoints versus base models (interesting in its own right — would suggest RLHF amplifies the human-pattern internalization).
...
The seed-lineage reading list — covering the prior art a reader needs to evaluate the plan — is: Andreas 2022 (user-modeling theory); Fiske/Cuddy/Glick/Xu 2002, Cuddy/Fiske/Glick 2008 (SCM grounding for the IV); EmotionPrompt / Li 2023, OPRO / Yang 2023, Salewski 2023 (framing-effects evidence); Sclar 2024 (prompt-sensitivity noise floor); Wei 2022 (chain-of-thought baseline); AGENTS.md spec and Karpathy 2025 (instruction-document baselines); MMLU / GSM8K / HumanEval / TruthfulQA / FEVER (the benchmark suite); Perez 2022 / Sharma 2023 (sycophancy context for the truthfulness-profile axes); Park 2024, Zou 2023 / RepE, Tigges 2023, Templeton 2024 (interpretability methodology and SAE-feature evidence); Choi 2025 / Transluce (closest variable-side peer on user-attribute decoding); Vig 2020, Geiger 2021/2022 (causal-mediation standard for Phase 3); Marks 2023, Arditi 2024 (causal-mediation worked examples on adjacent concepts); Binder 2024, Turpin 2023, Lindsey 2025 (introspection methodology for Phase 4).
