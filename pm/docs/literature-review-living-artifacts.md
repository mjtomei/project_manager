...
The plan's honest framing — "we study the prior work for inspiration and vocabulary, not as a blueprint" — turns out to be exactly the right posture. And the plan's central claim survives this survey intact. Stated plainly, that claim is: **we need a new data structure for artifacts — one that is non-deterministic, relational, and relies on intelligence to resolve conflicts between edits, or between an artifact and itself, or between an artifact and another artifact.** It is a vision people have gestured at before; what is new is that large language models finally make it buildable. The three closest recent works (CodeCRDT, SwarmSys, "Drop the Hierarchy") are academic demonstrations on narrower slices — none of them deployed in a real tool, none of them building the general artifact data structure the plan specifies. They corroborate the direction; they do not preempt it. The conclusion states this precisely.
...
The plan's framing of these four systems holds up, with one matter of emphasis. The plan's sentence "most of them assumed a central scheduler or arbiter" is true of blackboard systems and looser for the other three: actors, contract-net, and Linda are decentralized in the strict sense of "no master scheduler." But the plan's framing does not rest on the scheduler point alone — it pairs it with "boxed in by determinism and programmability constraints," and *that* is the load-bearing diagnosis, and it is exactly right for all four. Every one of these systems made the per-unit intelligence **small, fixed, and pre-programmed**, and negotiated over **scheduling or assignment, not over the content of the work**. The plan should keep both halves of its framing — the scheduler point is the weaker half, the determinism point is the strong one, and together they correctly describe why these systems could only gesture at what the plan proposes to build. Replacing the fixed per-unit program with a general reasoner, and moving the negotiation surface onto the work's content, is the genuine shift, and the plan names it.
...
So the plan is correct that the *mainstream* of multi-agent LLM work — and the commercial products built on it — is orchestrator-centric or fixed-pipeline. Its "no central arbiter" claim is a genuine departure from this dominant design. A small cluster of very recent academic work has begun probing the decentralized alternative (Section 5 covers it), which corroborates the plan's direction — but those are isolated demonstrations, not built substrates, and the orchestrator-centric design still owns the deployed tools. The plan's "no central arbiter" stays as its framing; Section 5 shows where the academic frontier is testing the same bet, and where it diverges.
...
- **"Drop the Hierarchy and Roles: How Self-Organizing LLM Agents Outperform Designed Structures" (arXiv:2603.28990, 2026)** is the most relevant paper for this plan. It is a 2026 arXiv study — **a controlled study, not a system**, and not deployed anywhere. It runs **25,000+ task runs** across 8 models, 4–256 agents, 8 coordination protocols, and 4 complexity levels, directly comparing centralized coordination, fully-autonomous self-organization, and hybrids. Its findings bear directly on the plan's bets:
  ...
  - A *hybrid* — fixed agent ordering, autonomous role selection — beat fully-autonomous coordination by 44% and centralized coordination by 14%. The plan reads this as a tuning result, not a refutation: the plan's "no central arbiter" is a claim about *who adjudicates conflicts* (no manager-intelligence sits above the artifact), not a claim that *no structure of any kind may exist*. Fixed ordering is a structural prior, not an arbiter; the plan's privileged-participant gradient is itself a deliberate, non-flat structure. "No central arbiter" stays as the plan's framing — the early PRs can absorb a structural prior like fixed ordering without giving up the no-arbiter commitment.
...
This last finding looks, at first read, like a threat to the plan's cost-stratification idea (§"Privileged participants" — "demote routine specializations to cheaper intelligence"). It is not, and the plan's framing already answers it. **The strong-model-privilege finding is a snapshot of a moving floor.** "Drop the Hierarchy" measured, at one moment in time, where the capability floor for competent self-organization sat — and at that moment it sat above the cheap tier. The plan's cost-stratification does not assume *today's* cheap models self-organize. It assumes the *self-organization floor descends as the frontier rises*: the same compute-cost trajectory that turned every previous frontier model into a routine, low-cost tier within a few years applies to today's frontier model too. The capability now needed for competent negotiation is, on that trajectory, a cheap tier in a few years. And the plan's privileged-participant safety net — humans, watchers, integrity-maintenance tasks — is exactly what carries the design through the interim while the floor is still descending. So the correct reading of "Drop the Hierarchy"'s threshold finding is not "cost-stratification is unsafe" but "cost-stratification is correctly staged": the plan staffs routine tasks with cheaper intelligence *as the floor reaches them*, with privileged participants covering the gap until it does. The plan should cite this paper and present the time-relativity rebuttal alongside it — not file the finding as an unanswered threat.
...
The plan's contribution is best stated in one sentence, and the whole survey above is the case for it: **the plan specifies and builds a new data structure for artifacts — one that is non-deterministic, relational, and relies on intelligence to resolve conflicts.** That is the claim. It is, deliberately, "something people have gestured at before, newly enabled by LLMs." This conclusion makes that case in four steps.
...
- **Non-deterministic.** Unlike a markdown file or a database row, the artifact's state is not a single fixed value. It holds proposals in superposition — in-flight, not-yet-resolved tasks converging through negotiation — as first-class state. The structure has to represent the unresolved, not just the resolved.
...
- **Intelligence-resolved.** Conflicts are resolved by LLM reasoning over the artifact-integrity constraint — no deterministic merge rule, no central arbiter. This is the part that was impossible before, and the part the prior visions could only gesture at.

And the plan proves it on a real substrate, in sequence: pm's plan files first, then pm's PR specifications, then pm's own orchestration state — a working project-management tool, not a research demo. Section 8's grounding is the same point in the deployed tool's own vocabulary: Claude Code has a static-config tier and a reactive-session tier and nothing between them; the living-artifacts data structure is the missing middle — a first-class, continuously co-authored, between-sessions artifact.

### The close prior art corroborates; it does not preempt

The three works nearest the plan are academic demonstrations on narrower slices, and not one of them saw commercial or real-world deployment. **CodeCRDT** does CRDT-plus-LLM coordination for code-merge specifically, and its 5–10% semantic-conflict measurement is the empirical motivation for the plan's intelligence-resolved layer — a separate group independently sizing the exact gap the plan fills. **SwarmSys** is a decentralized coordination architecture studied in isolation; it shows the coordination layer works, and stops there. **"Drop the Hierarchy"** is a controlled study, not a system; it supplies direct evidence that capable LLM agents self-organize productively, and its strong-model-privilege finding is a snapshot of a descending floor the plan's cost-stratification already accounts for. (None of the three should be confused with the commercial **Swarms** orchestration framework, which is an instance of the central-orchestration design the plan argues against.) Together these works are the strongest possible corroboration: independent groups, on narrower problems, are finding that the plan's direction works. What none of them does is build the general artifact data structure inside a working tool. That is the plan's job, and it is unclaimed.

### Honest read on the framing

The plan's framing survives this survey. Its four boldest moves — that prior coordination systems were boxed in by determinism, that there is no central arbiter, that living units open a different mode of evolutionary computation, that the "wants" scaffold is operational and not decorative — all hold, with the calibrations the body of this review supplies: the determinism diagnosis is the strong half of the prior-systems claim and the scheduler point the weaker half; "no central arbiter" is a claim about adjudication, compatible with structural priors; the evolutionary-computation direction inherits openly from Promptbreeder, FunSearch, AlphaEvolve and ADAS, with only *negotiated internal selection* held back as a narrow long-term sliver; and the "wants" scaffold is grounded in the persona-prompting literature with a cheap confirmatory experiment named. The contribution is not narrower than the headline — it *is* the headline, stated precisely: a non-deterministic, relational, intelligence-resolved data structure for artifacts, an old vision that LLMs finally make buildable, proven on pm itself.
