...
- **[[plan-mind]] — depends on `BranchRequest`.** The marker model's `<branch>/<emit>/<join>` is unbuildable without a first-class primitive for a stream to spawn a child with a dynamically-generated input and a return channel (the proposed `BranchRequest`, sibling of `AttentionRequest`; budget/depth-governed). `BranchTask.context_refs` = the `S_i`; the child's return emission = the `c_i`. *(Primitive proposed; not yet written into plan-mind.)*
