You are running QA scenario 25: "Bug-fix flow prompt structure for bug vs non-bug PRs (impl + review)"
...
**Focus**: Generated prompt for a plan == bugs PR contains the new 5-step Bug Fix Flow with correct captures path, references to pm/qa/instructions/ and pm/qa/artifacts/, and a review checklist that flags missing

**Steps**:
**Setup (from tui-manual-test.md):**

1. Install pm into an isolated venv and override PYTHONPATH so the editable clone is used (not `/opt/pm-src`):
```
...
2. Create a throwaway test project under the workdir (or `/tmp`):
```
...
3. Add the two PRs. `pm pr add` requires an existing plan id (hash like `plan-abc1234`) for `--plan`, and the bug-fix detection (`_is_bug_pr` in `pm_core/bug_fix_prompts.py`) keys off the literal string `"bugs"` in the PR's `plan` field — not off any user-visible "plan called bugs". So create both PRs first, then bootstrap-edit `pm/project.yaml` to set the bug PR's `plan` to the literal `bugs` (and leave the feature PR's `plan` empty/unset). This is the documented exception in tui-manual-test.md ("only for bootstrapping the initial test fixture").
```
...
# Note the two pr-XXXXXXX ids printed.
```
Then open `pm/project.yaml` and, on the bug PR's entry only, change/add `plan: bugs` (the literal seven-character string). Save. Do not touch other fields.

**Impl prompt — bug PR:**

4. Render the impl prompt via the user-facing CLI (the `pm prompt <pr-id>` command at `pm_core/cli/__init__.py:419` is the actual entry point — there is no `pm pr session --print-prompt` flag):
```
pm prompt <bug-pr-id> | tee /tmp/bug-impl-prompt.txt
```
Verify visibly in the output:
- (a) A `## Bug Fix Flow` section is present.
- (b) Five numbered steps appear in this exact order:
1. **Manual repro on pre-fix code**
2. **Write a failing test**
3. **Fix**
4. **Verify with the test**
5. **Verify manually**
- (c) Step 1's body mentions: reusing a valid `pm/qa/captures/<bug-pr-id>/impl/pre-fix/` capture if present; otherwise reproducing against pre-fix code via stash or parent-commit checkout / reverting fix files; and explicitly says "check in with the user" if reproduction doesn't work.
- (d) Both `pm/qa/instructions/` and `pm/qa/artifacts/` are referenced in step 1.
- (e) The captures path interpolates the PR's local id literally — search for `pm/qa/captures/<bug-pr-id>/impl/pre-fix/` and `pm/qa/captures/<bug-pr-id>/impl/post-fix/` (substituting the actual id you noted). The `<seg>` placeholder must NOT appear.

**Impl prompt — feature PR (negative case):**

5. Render the feature PR's impl prompt the same way and confirm no Bug Fix Flow block:
```
pm prompt <feature-pr-id> | tee /tmp/feat-impl-prompt.txt
grep -c "Bug Fix Flow" /tmp/feat-impl-prompt.txt   # expect 0
```

**Review prompt — bug PR:**

6. There is no `pm pr review --print-prompt` flag — `pm pr review` only builds the prompt then launches Claude (`pm_core/cli/pr.py:1170`). Render the review prompt directly through the same public function the CLI calls, using a one-liner that loads the real project state:
```
python -c "
from pathlib import Path
from pm_core import store, prompt_gen
data = store.load(Path('$TEST_DIR'))
print(prompt_gen.generate_review_prompt(data, '<bug-pr-id>'))
" | tee /tmp/bug-review-prompt.txt
```
Verify:
- A `## Bug Fix Review Checklist` section is present.
- The captures bullet references `pm/qa/captures/<bug-pr-id>/impl/` (literal id, no placeholder).
- Missing/unconvincing captures are flagged as `INPUT_REQUIRED` — not `NEEDS_WORK`. Grep for both to confirm:
```
grep -c INPUT_REQUIRED /tmp/bug-review-prompt.txt   # expect ≥1
grep -c NEEDS_WORK    /tmp/bug-review-prompt.txt   # expect 0 in this block
```
- The checklist also calls out: failing-then-passing test required (unless skipped via PR note), test fails for the right reason, and scope-limited-to-bug (flag drive-by refactors).

**Review prompt — feature PR (negative case):**

7. Same one-liner with the feature PR id; confirm `Bug Fix Review Checklist` does not appear:
```
python -c "
from pathlib import Path
from pm_core import store, prompt_gen
data = store.load(Path('$TEST_DIR'))
print(prompt_gen.generate_review_prompt(data, '<feature-pr-id>'))
" | grep -c "Bug Fix Review Checklist"   # expect 0
```

8. Record the entire session (commands + outputs of steps 4–7) following `cli-recording.md`, so the four captured prompts and the four greps are visible in the artifact.
...
`pm/qa/captures/pr-6be8ee6/scenarios/25/` (each recipe's
...
`pm/qa/captures/pr-6be8ee6/scenarios/25/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/25/`
- `git commit -m "qa: capture for scenario 25"`
