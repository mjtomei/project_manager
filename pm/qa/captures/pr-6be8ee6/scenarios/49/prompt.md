You are running QA scenario 49: "TUI QA pane — three sections, status counter, and create-picker modal routing"
...
**Focus**: The QA pane shows Instructions, Regression Tests, Artifact Recipes (even with zero items), the

**Steps**:
GIVEN: A throwaway pm project where pm is installed in a venv (`python3 -m venv /tmp/pm-venv && source /tmp/pm-venv/bin/activate && pip install -e .` from the project_manager clone, with `PYTHONPATH=/workspace` exported so the editable install isn't shadowed by `/opt/pm-src` — confirm with `pm which`). A throwaway project is created (`TEST_DIR=<workdir>/pm-test-$(date +%s); mkdir -p "$TEST_DIR" && cd "$TEST_DIR" && git init`), initialized with `pm init --backend local --no-import`, and seeded with a few PRs via `pm pr add`. Exactly one file is committed under each of `pm/qa/instructions/`, `pm/qa/regression/`, and `pm/qa/artifacts/` (each a markdown file with frontmatter `title:` and `description:` plus a short body). The TUI session is started from `$TEST_DIR` with `cd "$TEST_DIR" && pm session 2>/dev/null || true` (the attach error is expected and ignored — the tmux session is still created and the TUI is running in it).
WHEN: From outside the TUI session, simulate pressing `q` in the TUI pane via `tmux send-keys -t <session>:<window>.<pane> q ""` (find the pane with `tmux list-panes -t <session> -F '#{window_index}.#{pane_index} #{pane_current_command}'` or use `pm tui view -s <session>` to verify the TUI is the active pane).
THEN:
- `pm tui view -s <session>` shows the QA pane rendered with three labeled section headers — `Instructions (1)`, `Regression Tests (1)`, and `Artifact Recipes (1)` — each followed by a divider line and the single seeded item under it (showing `<id>: <title>` and the description).
- The bottom status bar reads `QA    3 item(s)    Enter=run  e=edit  d=debug  a=add  q=back` (the total is the sum across the three sections).

GIVEN: The QA pane is open as above and the cursor (`▸`) is on the regression item (use `tmux send-keys ... Down ""` as needed; the first selectable item is the lone Instructions entry, so one Down lands on the regression item).
WHEN: Send `Enter` to the TUI pane (`tmux send-keys -t <session>:<window>.<pane> Enter ""`).
THEN: A new pane is launched in the same tmux window running a `claude` invocation whose prompt is built by `build_regression_test_prompt`. Capturing that pane's scrollback with `tmux capture-pane -p -t <session>:<window>.<new-pane> -S -` shows the literal substring `pm/qa/captures/regression/<test-id>/<timestamp>/` (or with the regression id interpolated in place of `<test-id>`) somewhere in the prompt text.

GIVEN: The QA pane is open (return to it with `q` if a regression pane was launched, or focus the TUI pane).
WHEN: Send `a` to the TUI pane.
THEN: `pm tui view -s <session>` shows a centered modal titled `Create QA file` with a `Name` input field, a `Kind` section listing exactly three options (`Instruction`, `Regression test`, `Artifact recipe` — the TUI picker exposes only the guided `author-*` flow; the scaffold `add-*` flow is CLI-only), and a hint line `↑↓ change kind · Enter create · Esc cancel`. The selection pointer `▸` is on `Instruction` by default.

GIVEN: The create-picker modal is open with the Name input focused.
WHEN: Type `picker-test` into the Name input, press `Down` twice to land on `Artifact recipe`, then press `Enter`.
THEN: The modal dismisses and a new pane is launched in the current tmux window running the shell command `pm qa author-artifact picker-test` (verify with `tmux capture-pane -p -t <session>:<window>.<new-pane> -S -` showing the command line and/or the guided author flow output).

GIVEN: The QA pane is open again (the picker is closed).
WHEN: Send `a` to reopen the picker, then `Enter` without typing a name.
THEN: No new pane is launched. The modal either remains open with the empty input (the code returns early when `name` is blank) or no action occurs — `pm tui view` confirms no `pm qa author-*` pane appears, and the tmux window pane count is unchanged.

GIVEN: The create-picker modal is open (reopen with `a` if it was dismissed).
WHEN: Send `Escape` to the TUI pane.
THEN: The modal closes (the QA pane is visible again in `pm tui view`) and no new pane is launched.
...
under `pm/qa/captures/pr-6be8ee6/scenarios/49/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/49/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/49/`
- `git commit -m "qa: capture for scenario 49"`
