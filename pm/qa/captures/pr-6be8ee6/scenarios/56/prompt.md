You are running QA scenario 56: "Packaged qa_library.md survives pip install and frontmatter resilience"
...
**Focus**: pm qa docs outside the source tree, plus loader/CLI/TUI tolerance for legacy/missing frontmatter

**Steps**:
GIVEN: a fresh checkout of the pm repo with no prior pm install, where `./install.sh --local` has been run (creates `~/.local/share/pm/venv`, symlinks `~/.local/bin/pm`, and packages `pm_core/docs/qa_library.md` via `pyproject.toml`'s `[tool.setuptools.package-data] pm_core = ["docs/*.md"]`); `~/.local/bin` is on PATH; `PYTHONPATH` is unset (or scrubbed) so the installed copy is what runs; the working directory is an unrelated empty tmpdir with no `pm/project.yaml` anywhere on the path
WHEN: the user runs `pm qa docs`
THEN:
- exit code is 0
- stdout contains the packaged `qa_library.md` content (verify by checking that the printed output matches `~/.local/share/pm/venv/lib/python*/site-packages/pm_core/docs/qa_library.md` byte-for-byte via diff)
- no traceback is printed to stderr

GIVEN: the same `--local` install of pm; a fresh throwaway project created by `mkdir -p /tmp/pm-fm-test-$$ && cd /tmp/pm-fm-test-$$ && git init && pm init --backend local --no-import`; three hand-authored files placed under `pm/qa/instructions/`:
- `no-frontmatter.md` containing only a markdown body and no leading `---` delimiters
- `legacy-tags.md` containing valid `---` frontmatter with `title:`, `description:`, and a `tags: [foo, bar]` field (the loader still reads `tags` but it is no longer surfaced by `qa list`/`qa show`)
- `missing-description.md` containing valid `---` frontmatter with only `title:` (no `description:` key)
WHEN: the user runs `pm qa list`
THEN:
- exit code is 0
- the `Instructions` section lists three rows including `no-frontmatter`, `legacy-tags`, and `missing-description`
- `no-frontmatter` shows the file-id-derived title (`No Frontmatter`) with no `— <description>` suffix (since `_parse_frontmatter` returns `{}` and `_list_dir` falls back to the prettified stem)
- `missing-description` shows its title with no description suffix
- no Python traceback appears on stdout or stderr

WHEN: the user runs `pm qa show no-frontmatter`, then `pm qa show legacy-tags`, then `pm qa show missing-description`
THEN: each invocation
- exits 0
- prints a `# <title>` header, optional description line, the `[<path>]` line, and the file body
- for `no-frontmatter` the body equals the entire raw file contents (since the parser short-circuits when content does not start with `---`)
- no `FileNotFoundError`, no `yaml.YAMLError`, no traceback is emitted

GIVEN: the same throwaway project, with a running pm tmux session started by `cd /tmp/pm-fm-test-$$ && pm session 2>/dev/null || true` (attach fails in Bash because no TTY, but the tmux session is created and usable; inspect via `pm tui view` and `tmux send-keys`)
WHEN: the user presses `q` in the TUI (drive via `tmux send-keys -t <session>:<window>.<pane> q`), which fires the `toggle_qa` binding and calls `_refresh_qa_pane` → `qa_instructions.list_all` → `QAPane.update_items`
THEN:
- the QA pane opens and lists three Instructions rows for `no-frontmatter`, `legacy-tags`, and `missing-description`
- each row renders a header (title, falling back to prettified stem) without crashing the pane; missing descriptions simply omit the description line (the renderer uses `item.get("title", "Untitled")` and `item.get("description", "")`)
- `pm tui view` after the keypress shows the QA pane content with no Textual error banner or traceback overlay
...
under `pm/qa/captures/pr-6be8ee6/scenarios/56/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/56/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/56/`
- `git commit -m "qa: capture for scenario 56"`
