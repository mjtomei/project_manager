You are running QA scenario 32: "Bug-fix flow prompt is restructured for a bug PR and absent for non-bug PRs"
...
**Focus**: pm pr session / impl prompt for plan == bugs includes the new 5-step flow with the correct captures path, references to pm/qa/instructions/ and pm/qa/artifacts/, and the pre-fix reproduce gate; a non-bug

**Steps**:
## Setup (from instruction file)

1. Install pm into a venv from this clone and ensure it shadows the container's `/opt/pm-src`:
```
...
3. Add a bug PR and a non-bug (features) PR via the CLI. Note that `--plan` just records a string on the PR entry; the bug-fix flow triggers on `plan == "bugs"` (see `pm_core/bug_fix_prompts.py:13-20`), so no separate `pm plan add` is needed.
```
pm pr add 'demo bug' --plan bugs --description 'repro: clicking X crashes Y'
pm pr add 'demo feature' --plan features --description 'add Z'
```
Capture the two generated IDs (e.g. `BUG_ID`, `FEAT_ID`) from the "Created pr-XXXXXXX" output.

4. Start a pm tmux session for capture (the attach will fail with no TTY, that's fine):
```
...
5. **Print the bug PR's impl prompt** using the actual user entry point. The scenario draft's `pm pr session --dry-run` does not exist; the real command is `pm prompt <pr_id>` (see `pm_core/cli/__init__.py:419-450`), which calls `prompt_gen.generate_prompt` — the same function `pm pr start` uses to assemble the impl prompt at launch.
```
pm prompt "$BUG_ID" | tee /tmp/bug-impl-prompt.txt
```

6. **Verify the bug impl prompt contents.** All of the following must appear in `/tmp/bug-impl-prompt.txt`:
- Heading line `## Bug Fix Flow`
- Five numbered steps in order, recognizable by their bold labels:
1. `**Manual repro on pre-fix code**`
2. `**Write a failing test**`
3. `**Fix**`
4. `**Verify with the test**`
5. `**Verify manually**`
- The literal path `pm/qa/captures/<BUG_ID>/impl/pre-fix/` (with `<BUG_ID>` interpolated to the actual id from step 3)
- The literal path `pm/qa/captures/<BUG_ID>/impl/post-fix/`
- References to `pm/qa/instructions/` and `pm/qa/artifacts/`
- Explicit mention of `stash` and `check out the parent` (the pre-fix reproduction gate)
- The phrase `check in with the user` tied to "reproduction doesn't work"

Use `grep -F` for each substring; fail the scenario if any check misses. Example:
```
grep -F "## Bug Fix Flow" /tmp/bug-impl-prompt.txt
grep -F "pm/qa/captures/$BUG_ID/impl/pre-fix" /tmp/bug-impl-prompt.txt
grep -F "pm/qa/captures/$BUG_ID/impl/post-fix" /tmp/bug-impl-prompt.txt
grep -F "pm/qa/instructions/" /tmp/bug-impl-prompt.txt
grep -F "pm/qa/artifacts/" /tmp/bug-impl-prompt.txt
grep -E "stash|check out the parent" /tmp/bug-impl-prompt.txt
grep -F "check in with the user" /tmp/bug-impl-prompt.txt
```

7. **Print the non-bug PR's impl prompt** and verify the Bug Fix Flow block is *absent*:
```
pm prompt "$FEAT_ID" | tee /tmp/feat-impl-prompt.txt
! grep -F "## Bug Fix Flow" /tmp/feat-impl-prompt.txt
! grep -F "pm/qa/captures/$FEAT_ID/impl/pre-fix" /tmp/feat-impl-prompt.txt
```

8. **Print the bug PR's review prompt.** There is no CLI flag that prints the review prompt without launching tmux/Claude (see `pm_core/cli/pr.py:1322`+), so invoke the generator directly — this is the same `prompt_gen.generate_review_prompt` `pm pr review` calls at `pm_core/cli/pr.py:1170`:
```
python -c "
...
" | tee /tmp/bug-review-prompt.txt
```
Verify it contains a `## Bug Fix Review Checklist` heading and the line that classifies missing/unconvincing captures as `INPUT_REQUIRED` (not `NEEDS_WORK`):
```
grep -F "## Bug Fix Review Checklist" /tmp/bug-review-prompt.txt
grep -F "pm/qa/captures/$BUG_ID/impl/" /tmp/bug-review-prompt.txt
grep -F "INPUT_REQUIRED" /tmp/bug-review-prompt.txt
! grep -E "missing.*NEEDS_WORK|unconvincing.*NEEDS_WORK" /tmp/bug-review-prompt.txt
```

9. **Confirm the non-bug review prompt has no bug-fix block:**
```
python -c "
from pathlib import Path
from pm_core import store, prompt_gen
root = Path('$TEST_DIR')
data = store.load(root)
print(prompt_gen.generate_review_prompt(data, '$FEAT_ID'))
" | tee /tmp/feat-review-prompt.txt
! grep -F "## Bug Fix Review Checklist" /tmp/feat-review-prompt.txt
```

10. **Capture artifacts.** Save the four prompt dumps and an asciinema of the CLI invocations under the repo's QA captures dir:
```
DEST=/workspace/pm/qa/captures/pr-6be8ee6/scenarios/32
mkdir -p "$DEST"
cp /tmp/bug-impl-prompt.txt /tmp/feat-impl-prompt.txt \
/tmp/bug-review-prompt.txt /tmp/feat-review-prompt.txt "$DEST/"
asciinema rec "$DEST/run.cast" -c "bash -lc 'pm prompt $BUG_ID; pm prompt $FEAT_ID'"
```
Record `$BUG_ID` and `$FEAT_ID` in a short `notes.md` in `$DEST` so the captures are reproducible.

11. Scenario passes only if every grep in steps 6, 7, 8, 9 succeeds (and the negated greps return no match). Any miss → record which assertion failed in `$DEST/notes.md` and report `NEEDS_WORK`.
...
`pm/qa/captures/pr-6be8ee6/scenarios/32/` (each recipe's
...
`pm/qa/captures/pr-6be8ee6/scenarios/32/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/32/`
- `git commit -m "qa: capture for scenario 32"`
