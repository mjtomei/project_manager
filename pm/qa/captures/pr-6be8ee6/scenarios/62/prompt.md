You are running QA scenario 62: "pm qa regression replaces pm tui test with unified filing addendum"
...
- QA INPUT_REQUIRED: Bug-fix flow prompt structure varies by PR plan: PASS; pm qa add-/author-/list/show/edit across three categories: PASS; pm qa regression replaces pm tui test with unified filing addendum: PASS; TUI QA pane shows three sections and create-picker modal routes correctly: PASS; Packaged qa_library.md survives pip install and frontmatter resilience: PASS; End-to-end bug-fix session produces pre-fix + post-fix captures and downstream QA scenarios save under the new layout: INPUT_REQUIRED; Review-loop INPUT_REQUIRED gate and duplicate-verifier guard: INPUT_REQUIRED (workdir: /home/matt/.pm/workdirs/qa/pr-6be8ee6-5b3236ea0e767f20) (2026-05-13T21:40:00Z)
...
**Focus**: pm qa regression <id> runs the unified regression prompt; --file-prs and the hidden --file-bugs alias both produce the same bugs+improvements addendum; old pm tui test is gone

**Steps**:
GIVEN: A throwaway pm project set up per the instruction file's Setup section — a fresh venv with `pip install -e .` from the project_manager clone, `PYTHONPATH=/workspace` exported (confirm with `pm which` printing your clone, not `/opt/pm-src`), a test dir under workdir or `/tmp` initialized with `git init` and `pm init --backend local --no-import`, a handful of PRs added via `pm pr add`, a pm tmux session started from the test dir via `cd "$TEST_DIR" && pm session 2>/dev/null || true` (ignore the no-TTY attach error), and a minimal regression file created with `pm qa add-regression smoke` then edited to contain a one-line trivial body (e.g. `Just echo "hello" and stop.`)
WHEN: From a new pane inside the test tmux session (use `tmux send-keys` from outside), the user runs `pm qa regression smoke` with no flag, then `pm qa regression smoke --file-prs`, then `pm qa regression smoke --file-bugs`, and after each invocation runs `pm log grep -- "qa-regression"` (or `pm log show -n 500`) to retrieve the actual `claude … <prompt>` command line that `launch_claude` wrote via `log_shell_command` before spawning claude. (Claude itself will not run usefully under the Bash tool's no-TTY conditions; we only need the logged prompt argument, which is recorded prior to `subprocess.run`.)
THEN:
- The no-flag invocation's logged prompt contains the regression body and Session Context but no "Filing Findings" addendum and no `--plan bugs` / `--plan improvements` text
- The `--file-prs` and `--file-bugs` invocations produce byte-identical filing-addendum text in their logged prompts, and that addendum contains both `pm pr add … --plan bugs …` and `pm pr add … --plan improvements …` filing instructions
- `--file-bugs` is accepted silently (hidden alias) — no deprecation warning, no error
WHEN: From a pane in the test session, the user runs `pm qa regression does-not-exist`
THEN: The command exits non-zero (rc=1) and prints `Unknown regression test: does-not-exist` on stderr plus `Run 'pm qa list' to see available tests.` on stdout, with no Python traceback
GIVEN: The same throwaway project but with the pm tmux session killed (`tmux kill-session -t <name>`) so no pm session exists
WHEN: The user runs `pm qa regression smoke` from a plain shell
THEN: The command exits non-zero (rc=1) and prints `No pm tmux session found. Start one with 'pm session'.` on stderr, with no Python traceback
GIVEN: The same project (session state irrelevant for this triple)
WHEN: The user runs each of `pm tui test`, `pm tui test --list`, `pm tui test --file-bugs`, `pm tui test --fix-bugs`
THEN: Each command exits non-zero with a Click "No such command 'test'." usage error referencing the `pm tui` group — the `test` subcommand is absent from `pm_core/cli/tui.py`, so Click rejects the command before any option parsing; no stack trace, no regression run
...
under `pm/qa/captures/pr-6be8ee6/scenarios/62/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/62/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/62/`
- `git commit -m "qa: capture for scenario 62"`
