You are running QA scenario 13: "QA planner artifact block + parse_qa_plan + install_artifact_files end-to-end"
...
**Focus**: Artifact recipe surfacing in planner prompt, ARTIFACT field parsing (valid/bogus/cross-category/comma list/none), and _install_artifact_files

**Steps**:
**Setup**
1. `cd /workspace` and confirm `pm/qa/artifacts/tmux-screen-recording.md` and `pm/qa/artifacts/cli-recording.md` exist; confirm `pm/qa/instructions/tui-manual-test.md` exists (used as the cross-category test case). PR id under test: `pr-6be8ee6`.
2. Make the scenario dir: `mkdir -p pm/qa/captures/pr-6be8ee6/scenarios/13/`.

**Write the pytest** — create `/tmp/test_qa_artifact_e2e.py` with these test functions (all in one file; use the real project root at `/workspace` via `store.find_project_root` rather than synthesizing one, since the planner reads the real `pm/qa/artifacts/` library):

3. `test_planner_prompt_has_artifact_block`:
- Build a minimal `data` dict (project + one PR with `id="pr-001"`, `branch`, `title`, `description`) mirroring `_make_state` in `tests/test_pr_notes.py`.
- Patch `pm_core.prompt_gen.store.find_project_root` to return `Path("/workspace")` (so `list_artifacts` reads the real library).
- Call `prompt = generate_qa_planner_prompt(data, "pr-001")`.
- Assert `"Artifact Recipes"` is in `prompt` and the literal string `'ARTIFACT:'` appears in the SCENARIO template guidance (around line 1425/1429 of prompt_gen.py).

4. `test_planner_prompt_omits_artifact_block_when_empty`:
- Create a temp project root via `tmp_path` (pytest fixture) containing only `pm/qa/instructions/` with one .md file and no `pm/qa/artifacts/` dir (or empty one).
- Patch `store.find_project_root` to return `tmp_path`.
- Call `generate_qa_planner_prompt(data, "pr-001")`.
- Assert `"Artifact Recipes"` is NOT in the prompt and the `ARTIFACT:` SCENARIO-template line is not present (the conditional `if has_artifact_recipes` branches in prompt_gen.py drop both).
- (No need to physically move the real `/workspace/pm/qa/artifacts/*.md` files — using an isolated tmp_path is safer and avoids mutating the repo.)

5. `test_parse_qa_plan_artifact_field`:
- Build a `plan_text` containing `QA_PLAN_START` … `QA_PLAN_END` with four scenarios:
- SCENARIO 1 — `ARTIFACT: tmux-screen-recording.md, cli-recording.md`
- SCENARIO 2 — `ARTIFACT: tmux-screen-recording.md, bogus-recipe.md`
- SCENARIO 3 — `ARTIFACT: tui-manual-test.md` (cross-category — lives in `instructions/`)
- SCENARIO 4 — `ARTIFACT: none`
- Each scenario also needs a `FOCUS:` and `STEPS:` line so the parser keeps it; titles must not be `Scenario Title` / `...` (those get filtered).
- Call `scenarios = parse_qa_plan(plan_text, pm_root=Path("/workspace"))`.
- Assert `scenarios[0].artifact_paths == ["artifacts/tmux-screen-recording.md", "artifacts/cli-recording.md"]`.
- Assert `scenarios[1].artifact_paths == ["artifacts/tmux-screen-recording.md"]` (bogus dropped).
- Assert `scenarios[2].artifact_paths == []` (cross-category dropped — code at `qa_loop.py:631-638` warns and skips when resolved category != `"artifacts"`).
- Assert `scenarios[3].artifact_paths == []` (the `none` short-circuit at `qa_loop.py:622`).
- Use `caplog` (`with caplog.at_level(logging.WARNING, logger="pm_core.qa_loop")`) and assert warnings mentioning `bogus-recipe.md` and `tui-manual-test.md` were emitted.

6. `test_install_artifact_files_copies_and_rewrites`:
- From the parsed `scenarios[0]` (artifact_paths length 2), call `_install_artifact_files(Path("/workspace"), scenario, scratch_path=tmp_path, scratch_dir="/agent-scratch")`.
- Assert `(tmp_path / "qa-artifacts" / "tmux-screen-recording.md").is_file()` and same for `cli-recording.md`.
- Assert `scenario.artifact_paths == ["/agent-scratch/qa-artifacts/tmux-screen-recording.md", "/agent-scratch/qa-artifacts/cli-recording.md"]` (note: `scratch_dir` must be passed as a string — if omitted, paths become literal `"None/qa-artifacts/..."`).

7. `test_child_prompt_renders_artifact_section`:
- Using the same scenario (now with rewritten absolute artifact_paths), call `generate_qa_child_prompt(data, "pr-001", scenario, workdir="/tmp/workdir", scratch_dir="/agent-scratch")`.
- Assert `"## Artifact Capture Recipes"` (plural — `len(artifact_paths)==2` ⇒ "Recipes") is in the prompt.
- Assert both absolute paths render as bulleted lines: `` "- `/agent-scratch/qa-artifacts/tmux-screen-recording.md`" `` and the cli-recording one.
- Assert `"pm/qa/captures/pr-001/scenarios/1/"` appears (captures dir uses `pr_id` and `scenario.index`).
- Assert all three git lines appear: `git add pm/qa/captures/pr-001/scenarios/1/`, `git commit -m "qa: capture for scenario 1"`, `git push origin <branch>` — and the rebase-on-conflict line `git pull --rebase origin <branch> && git push origin <branch>`.

**Run + save**
8. From `/workspace`, run: `pytest -xvs /tmp/test_qa_artifact_e2e.py 2>&1 | tee pm/qa/captures/pr-6be8ee6/scenarios/13/pytest.log`. Verify all 5 tests pass.
9. Capture an asciinema cast of the run: `asciinema rec pm/qa/captures/pr-6be8ee6/scenarios/13/run.cast -c "pytest -xvs /tmp/test_qa_artifact_e2e.py"` (or follow `pm/qa/artifacts/cli-recording.md` if asciinema isn't available — fall back to the tee'd log + a `## Files` manifest).
10. Write `pm/qa/captures/pr-6be8ee6/scenarios/13/manifest.md` per the cli-recording recipe's manifest format, cross-linking the log and cast, and `git add / commit -m "qa: capture for scenario 13" / push origin <branch>` (rebase+retry on conflict).
...
`pm/qa/captures/pr-6be8ee6/scenarios/13/` (each recipe's
...
`pm/qa/captures/pr-6be8ee6/scenarios/13/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/13/`
- `git commit -m "qa: capture for scenario 13"`
