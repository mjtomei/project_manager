You are running QA scenario 61: "pm qa docs and packaging survive install from outside the source tree"
...
**Focus**: Packaged qa_library.md is shipped via setuptools.package-data and pm qa docs works from any cwd

**Steps**:
GIVEN: A fresh Python venv where pm has been installed via `./install.sh --local` from the project_manager source checkout (this creates a venv at `~/.local/share/pm/venv` and a `~/.local/bin/pm` symlink — a true non-editable install so packaged data is copied into site-packages, not loaded from the source tree). The user's PATH includes `~/.local/bin`, `PYTHONPATH` is unset (so the install isn't shadowed by the source tree), and the cwd is `/tmp/somewhere-unrelated` containing no `pm/` directory or `project.yaml`. Confirm with `pm which` that it resolves to the installed venv path, not the source checkout.
WHEN: The user runs `pm qa docs` from that cwd.
THEN:
- stdout contains the full packaged `pm_core/docs/qa_library.md` reference (verifiable by checking for distinctive headings from that file, e.g. the first heading line)
- the process exits 0
- stderr is empty — no `FileNotFoundError`, `Traceback`, or "No such file or directory" referencing `qa_library.md` or `docs/`

GIVEN: A pm project initialized in a temp dir (`pm init --backend local --no-import`) with three hand-crafted files staged under `pm/qa/instructions/`:
- `legacy.md` — full frontmatter block with `title:`, `description:`, and an obsolete `tags: [foo, bar]` field (the parser still reads `tags` but nothing references it downstream)
- `nodesc.md` — frontmatter block with `title:` but no `description:` field
- `nofrontmatter.md` — plain markdown body with no `---` delimiters at all (parser must fall back to empty metadata and filename-derived title)
WHEN: The user runs `pm qa list`, then `pm qa show legacy`, `pm qa show nodesc`, and `pm qa show nofrontmatter` from the project root.
THEN:
- `pm qa list` exits 0 and lists all three files under the Instructions section; `nodesc` and `nofrontmatter` appear with no `—` description suffix (because `description` defaults to ""); `nofrontmatter`'s title falls back to the Title-Cased filename stem
- each `pm qa show <id>` exits 0, prints `# <title>` then the `[<path>]` line and the body; for `nodesc` and `nofrontmatter` the description line is omitted rather than printing a blank or `None`
- none of the four commands emits a Python traceback on stderr
...
under `pm/qa/captures/pr-6be8ee6/scenarios/61/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/61/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/61/`
- `git commit -m "qa: capture for scenario 61"`
