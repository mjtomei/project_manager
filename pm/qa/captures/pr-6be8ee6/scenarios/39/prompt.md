You are running QA scenario 39: "pm qa regression replaces pm tui test with unified filing addendum"
...
**Focus**: pm qa regression <id> prompt assembly, --file-prs vs hidden --file-bugs alias, missing-tmux-session error, unknown id error, removal of pm tui test

**Steps**:
## Setup

1. Create an isolated venv and editable install of pm; ensure the editable clone shadows any container-baked install:
```
...
3. Start a pm tmux session from the test directory (ignore the attach error — the session is still created):
```
cd "$TEST_DIR" && pm session 2>/dev/null || true
tmux ls   # confirm a pm-* session for this dir is running
```

4. Install a stub `claude` binary earlier on PATH so the launched prompt is captured to a file instead of starting a real Claude session:
```
STUB_DIR=$(mktemp -d)
cat > "$STUB_DIR/claude" <<'SH'
#!/usr/bin/env bash
# Last positional arg is the prompt (per launch_claude cmd assembly).
for a in "$@"; do last="$a"; done
printf '%s' "$last" > /tmp/pm-qa-regression-prompt.txt
exit 0
SH
chmod +x "$STUB_DIR/claude"
export PATH="$STUB_DIR:$PATH"
command -v claude   # confirm it points at the stub
```

## Test Steps

5. Confirm `pm tui test` has been removed — these three should all fail with a click "No such command/option" error and exit non-zero (no Python tracebacks):
```
pm tui test                ; echo "rc=$?"
pm tui test --file-bugs    ; echo "rc=$?"
pm tui test --fix-bugs     ; echo "rc=$?"
```
Expect stderr text like `Error: No such command 'test'` and `rc != 0`.

6. Create a regression file with a trivial body (use `EDITOR=true` so add-regression doesn't block on $EDITOR, then overwrite with a minimal body):
```
EDITOR=true pm qa add-regression demo-reg
cat > pm/qa/regression/demo-reg.md <<'MD'
---
title: Demo Reg
description:
---
Trivial body for prompt-assembly check.
MD
pm qa list   # confirm demo-reg appears under regression
```

7. Unknown-id error path — must exit non-zero with a clear "Unknown regression test" message (not a stack trace):
```
pm qa regression nonexistent-id ; echo "rc=$?"
```
Expect stderr `Unknown regression test: nonexistent-id` plus the `Run 'pm qa list'` hint, `rc != 0`.

8. Happy path with no filing flag — capture the prompt the launcher hands to claude and verify it has NO filing addendum, mentions the captures path, and was assembled by `build_regression_test_prompt`:
```
rm -f /tmp/pm-qa-regression-prompt.txt
pm qa regression demo-reg ; echo "rc=$?"
PROMPT=$(cat /tmp/pm-qa-regression-prompt.txt)
echo "$PROMPT" | grep -q "## Session Context"              # from build_regression_test_prompt
echo "$PROMPT" | grep -q "## Captures"                     # from build_regression_test_prompt
echo "$PROMPT" | grep -q "pm/qa/captures/regression/<test-id>/<timestamp>/"
echo "$PROMPT" | grep -q "## QA Regression Test: Demo Reg"
echo "$PROMPT" | grep -q "Trivial body for prompt-assembly check."
! echo "$PROMPT" | grep -q "## Filing Findings"            # addendum MUST be absent
! echo "$PROMPT" | grep -q "Filing is independent of your verdict"
```

9. Missing-tmux-session error path — kill the pm session, rerun, confirm clean error exit:
```
PM_SESS=$(tmux ls -F '#{session_name}' | grep '^pm-' | head -1)
tmux kill-session -t "$PM_SESS"
pm qa regression demo-reg ; echo "rc=$?"
```
Expect stderr `No pm tmux session found. Start one with 'pm session'.` and `rc != 0`. No stack trace.

10. Restart tmux session, then run with `--file-prs`. Confirm the filing addendum is present and mentions BOTH `--plan bugs` AND `--plan improvements`:
```
cd "$TEST_DIR" && pm session 2>/dev/null || true
rm -f /tmp/pm-qa-regression-prompt.txt
pm qa regression demo-reg --file-prs ; echo "rc=$?"
PROMPT_PRS=$(cat /tmp/pm-qa-regression-prompt.txt)
echo "$PROMPT_PRS" | grep -q "## Filing Findings"
echo "$PROMPT_PRS" | grep -q -- "--plan bugs"
echo "$PROMPT_PRS" | grep -q -- "--plan improvements"
echo "$PROMPT_PRS" | grep -q "Filing is independent of your verdict"
```

11. Hidden alias `--file-bugs` produces an IDENTICAL filing addendum (single unified addendum covers both plans). Compare the two captured prompts byte-for-byte:
```
rm -f /tmp/pm-qa-regression-prompt.txt
pm qa regression demo-reg --file-bugs ; echo "rc=$?"
PROMPT_BUGS=$(cat /tmp/pm-qa-regression-prompt.txt)
echo "$PROMPT_BUGS" | grep -q "## Filing Findings"
echo "$PROMPT_BUGS" | grep -q -- "--plan bugs"
echo "$PROMPT_BUGS" | grep -q -- "--plan improvements"
diff <(printf '%s' "$PROMPT_PRS") <(printf '%s' "$PROMPT_BUGS")   # must produce no output
```
Also confirm `--file-bugs` is hidden from help (alias only): `pm qa regression --help | grep -- --file-bugs` should print nothing, while `pm qa regression --help | grep -- --file-prs` should show the documented flag.

## Reporting

For each numbered step, report PASS/FAIL with one line of evidence (the exact stderr line, the matched grep, or the diff result). End with overall PASS/FAIL.
...
`pm/qa/captures/pr-6be8ee6/scenarios/39/` (each recipe's
...
`pm/qa/captures/pr-6be8ee6/scenarios/39/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/39/`
- `git commit -m "qa: capture for scenario 39"`
