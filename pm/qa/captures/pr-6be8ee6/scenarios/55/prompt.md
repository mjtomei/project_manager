You are running QA scenario 55: "TUI QA pane shows three sections and create-picker modal routes correctly"
...
**Focus**: pressing q opens a three-section pane, status counter sums all three, pressing a opens the picker, Enter on regression launches the unified prompt

**Steps**:
GIVEN: a throwaway pm project at $TEST_DIR set up as follows:
- `python3 -m venv /tmp/pm-venv && source /tmp/pm-venv/bin/activate`
- From the project_manager clone: `pip install -e .`
- `export PYTHONPATH=/workspace` (override container's `/opt/pm-src`); confirm with `pm which`
- `TEST_DIR=<workdir>/pm-test-$(date +%s) && mkdir -p "$TEST_DIR" && cd "$TEST_DIR" && git init`
- `pm init --backend local --no-import`
- Add a couple of PRs via `pm pr add` so the tree is non-empty
- Author at least one file in each of `pm/qa/instructions/`, `pm/qa/regression/`, and `pm/qa/artifacts/` (a minimal `.md` with `title:` frontmatter is enough; or use `pm qa add-instruction`, `pm qa add-regression`, `pm qa add-artifact` to scaffold stubs from the CLI)
- Start the TUI in tmux: `cd "$TEST_DIR" && pm session 2>/dev/null || true` (attach error is expected — session is created)
- Inspect/drive via `pm tui view -s <session>` and `pm tui send <keys> -s <session>` (or `tmux send-keys`/`capture-pane`)
WHEN: the user presses `q` to open the QA pane (pane is currently hidden)
THEN:
- the pane shows three section headers in order: `Instructions (N)`, `Regression Tests (N)`, `Artifact Recipes (N)`, each with its items rendered beneath
- the status bar reads `QA    <total> item(s) …` where `<total>` equals the sum of the three section counts (per `_update_qa_status_bar` in pm_core/tui/app.py)

GIVEN: the TUI from the prior step, with the QA pane open
WHEN: the user closes the QA pane (press `q`), empties `pm/qa/artifacts/` (e.g. `rm pm/qa/artifacts/*.md`), then presses `q` again to reopen
THEN: the `Artifact Recipes (0)` header is still rendered (not hidden); the status-bar total drops accordingly

GIVEN: the QA pane is open and focused
WHEN: the user presses `a`, leaves the name Input empty, and presses Enter
THEN: the picker modal stays open with no crash (the empty-name submit is a no-op in `QACreatePickerScreen.on_input_submitted`); no new file appears under `pm/qa/`

GIVEN: the QA pane is open and the create-picker modal is open
WHEN: the user types a name (e.g. `demo-recipe`), uses ↓ arrows to highlight the `Artifact recipe` option, and presses Enter
THEN:
- the modal dismisses and a new tmux pane (window `qa-author`) opens running `pm qa author-artifact demo-recipe` — verify via `tmux list-panes`/`capture-pane` that the command appears in pane scrollback

GIVEN: the QA pane is open, focused, and contains at least one regression test item
WHEN: the user navigates (`j`/`down`) to a `regression:*` item and presses Enter
THEN: a new pane opens running `claude` with the prompt built by `pm_core.regression_prompts.build_regression_test_prompt`; the scrollback (or the prompt text passed on the command line) contains the captures-section text referencing `pm/qa/captures/regression/<test-id>/<timestamp>/`

GIVEN: the QA pane is open and the create-picker modal is open (any name typed or empty)
WHEN: the user presses Esc
THEN: the modal closes (dismissed via `action_cancel` → `None`); no pane is launched, no file is created under `pm/qa/`
...
under `pm/qa/captures/pr-6be8ee6/scenarios/55/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/55/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/55/`
- `git commit -m "qa: capture for scenario 55"`
