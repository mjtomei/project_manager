You are running QA scenario 23: "Packaged qa_library.md survives pip install and pm qa docs works from any cwd"
...
**Focus**: Confirm pyproject.toml's package-data wiring actually ships pm_core/docs/qa_library.md, and that pm qa docs from an unrelated working directory (no pm/ tree present) prints the full reference doc without

**Steps**:
1. From the pm source checkout at /workspace, create a fresh venv outside the repo and activate it:
```
python3 -m venv /tmp/pm-pkg-venv
source /tmp/pm-pkg-venv/bin/activate
```
2. IMPORTANT: the container sets `PYTHONPATH=/opt/pm-src` which shadows site-packages and would make this test meaningless. Unset it for the rest of the session so the *installed* pm_core is the one being exercised:
```
unset PYTHONPATH
```
3. From /workspace, install pm into the venv as a built wheel (not editable — we are validating package-data shipping). Prefer `pip install .` over `./install.sh --local`, since `--local` writes to `~/.local/share/pm/venv` and bypasses the active venv:
```
cd /workspace && pip install .
```
Confirm the active `pm` resolves into the venv: `which pm` should print `/tmp/pm-pkg-venv/bin/pm`, and `pm which` (the pm subcommand) should report the installed pm_core path, not `/workspace` or `/opt/pm-src`.
4. Verify the packaged doc file landed in site-packages:
```
python -c "import pm_core, pathlib; p = pathlib.Path(pm_core.__file__).parent / 'docs' / 'qa_library.md'; print(p); print('exists=', p.exists(), 'size=', p.stat().st_size if p.exists() else 0)"
```
Expect a path under `/tmp/pm-pkg-venv/lib/python*/site-packages/pm_core/docs/qa_library.md`, `exists=True`, and a non-zero size (the file in-tree is ~hundreds of lines).
5. cd to a directory with no pm/ tree and run `pm qa docs`:
```
cd /tmp && pm qa docs
```
Expect exit code 0. The first line of stdout must be `# pm QA library`, and the output must contain the section heading `## The four directories` (the planner called it a "table" — it is actually an H2 section, not a markdown table). Capture exit status with `echo "exit=$?"`.
6. Also confirm `pm qa docs` works when piped (no pager, no TTY assumptions):
```
cd /tmp && pm qa docs | head -20
cd /tmp && pm qa docs | wc -l   # should be well over 50 lines
```
7. Negative control: locate and delete the installed doc, then re-run `pm qa docs` to see how the failure surfaces:
```
...
```
`pm_core/qa_authoring.py:15` does a bare `_DOC_PATH.read_text()` with no try/except, so a raw `FileNotFoundError` traceback is the *expected* current behavior. If that's what you see, capture it verbatim — this is a finding (file a follow-up bug PR for a cleaner error message). If it surfaces a friendly error instead, capture that and note the discrepancy with the source.
8. Restore the installed file for cleanup so subsequent scenarios reusing this venv aren't broken:
```
cp /workspace/pm_core/docs/qa_library.md "$DOC"
cd /tmp && pm qa docs | head -1   # sanity: back to "# pm QA library"
```
9. Save the asciinema recording and transcript per pm/qa/instructions/cli-recording.md under `pm/qa/captures/pr-6be8ee6/scenarios/23/` (cast file + plaintext transcript covering steps 3–8, including the negative-control traceback).
10. Deactivate and remove the throwaway venv when done: `deactivate && rm -rf /tmp/pm-pkg-venv`.

Note on the included instruction file (TUI Manual Testing): its tmux/`pm session`/test-project bootstrap is not applicable to this scenario, which exercises a packaged-data + CLI-only path with no pm project in cwd. The one piece that *is* load-bearing — the `PYTHONPATH=/opt/pm-src` shadowing warning — is incorporated as step 2; the rest is intentionally skipped.
...
`pm/qa/captures/pr-6be8ee6/scenarios/23/` (each recipe's
...
`pm/qa/captures/pr-6be8ee6/scenarios/23/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/23/`
- `git commit -m "qa: capture for scenario 23"`
