You are running QA scenario 45: "Bug-fix flow prompt structure for a bug PR vs non-bug PR"
...
**Focus**: The impl prompt produced for a bugs-plan PR carries the 5-step bug-fix flow, references the

**Steps**:
GIVEN: A throwaway pm project where pm is installed in an isolated venv and exercises the local clone, containing two PRs — one filed under `--plan bugs` and one filed under `--plan improvements`, each with a short description. Concretely:
- `python3 -m venv /tmp/pm-venv && source /tmp/pm-venv/bin/activate && pip install -e .` from the project_manager clone, then `export PYTHONPATH=/workspace` and confirm with `pm which` that the editable clone wins over `/opt/pm-src`.
- `TEST_DIR=<workdir>/pm-test-$(date +%s); mkdir -p "$TEST_DIR" && cd "$TEST_DIR" && git init`.
- `pm init --backend local --no-import`.
- `pm pr add "Fix login crash on empty password" --plan bugs --description "App crashes when password field is empty"` — note the printed `pr-XXXXXXX` id as `BUG_ID`.
- `pm pr add "Refactor settings panel layout" --plan improvements --description "Reorganize tabs for clarity"` — note the id as `FEAT_ID`.
- Start the test tmux session: `cd "$TEST_DIR" && pm session 2>/dev/null || true` (ignore the attach error; the session is created and usable from another pane).
WHEN: From a new pane inside the test tmux session, the user runs `pm prompt $BUG_ID` to render the implementation prompt for the bug PR without launching Claude.
THEN:
- The output contains a `## Bug Fix Flow` section header.
- It contains five numbered steps in this order: (1) Manual repro on pre-fix code, (2) Write a failing test, (3) Fix, (4) Verify with the test, (5) Verify manually.
- Step 1 explicitly mentions reusing an existing pre-fix capture if one is valid, falling back to reproducing against pre-fix code by stashing changes or checking out the parent commit / temporarily reverting fix files, and checking in with the user when the bug doesn't reproduce.
- The captures paths interpolated in the prompt are literally `pm/qa/captures/<BUG_ID>/impl/pre-fix/` and `pm/qa/captures/<BUG_ID>/impl/post-fix/` (using the actual id from `pm pr add`, not a placeholder or any other PR's id).
- The prompt names both `pm/qa/instructions/` (env-setup recipes) and `pm/qa/artifacts/` (capture recipes) as directories to consult.

GIVEN: The same project, with `BUG_ID` and `FEAT_ID` as above.
WHEN: From a pane inside the test tmux session, the user runs `pm prompt $FEAT_ID` to render the implementation prompt for the features-plan PR.
THEN: The output contains no `## Bug Fix Flow` section and none of the five bug-fix-flow steps (no mention of pre-fix/post-fix captures, no five-step manual-repro flow).

GIVEN: The same project; `BUG_ID` exists with `plan: bugs`. (There is no user-facing CLI that prints the review prompt without launching Claude, so the review prompt is observed via the prompt file that `pm pr review` writes into the workdir before invoking Claude.)
WHEN: From a pane inside the test tmux session, the user advances `BUG_ID` to in_review and launches a background review window so the review prompt is materialized:
- `pm pr start $BUG_ID --background` (creates the workdir and review-eligible state).
- `pm pr review $BUG_ID --background`.
- The user then reads the prompt file Claude was invoked with: `cat $(ls -t <bug-workdir>/pm/prompts/pm_prompt_*.txt | head -1)` where `<bug-workdir>` is the workdir reported by `pm pr list --workdirs` for `BUG_ID`.
THEN: The rendered review prompt contains a `## Bug Fix Review Checklist` line stating that missing or unconvincing pre-fix / post-fix captures under `pm/qa/captures/<BUG_ID>/impl/` should be surfaced as **INPUT_REQUIRED** (explicitly INPUT_REQUIRED, not NEEDS_WORK), leaving the accept-without-capture decision to the user.
...
under `pm/qa/captures/pr-6be8ee6/scenarios/45/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/45/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/45/`
- `git commit -m "qa: capture for scenario 45"`
