You are running QA scenario 48: "pm qa regression — id resolution, filing addendum, and removed pm tui test"
...
**Focus**: The replacement pm qa regression <id> command — non-existent id error, no-session error,

**Steps**:
GIVEN: A throwaway pm project set up per the TUI manual testing instructions:
- Create venv and install pm: `python3 -m venv /tmp/pm-venv && source /tmp/pm-venv/bin/activate && pip install -e .` from the project_manager clone, then `export PYTHONPATH=/workspace` and confirm with `pm which`.
- Make a throwaway dir: `TEST_DIR=<workdir>/pm-test-$(date +%s); mkdir -p "$TEST_DIR" && cd "$TEST_DIR" && git init`.
- Initialize pm: `pm init --backend local --no-import`, then add at least one PR (`pm pr add "Add login feature"`).
- Create `pm/qa/regression/echo-smoke.md` with YAML frontmatter (`title: Echo Smoke`) and a trivial body (e.g. "Run `echo hello` and confirm it prints `hello`."). No pm tmux session is started.
WHEN: From `$TEST_DIR`, run `pm qa regression nonexistent-id`.
THEN: The command exits non-zero (exit 1) and prints `Unknown regression test: nonexistent-id` on stderr followed by `Run 'pm qa list' to see available tests.` (id resolution happens before the tmux-session check).

GIVEN: The pm project above with `pm/qa/regression/echo-smoke.md` present, a pm tmux session started via `cd "$TEST_DIR" && pm session 2>/dev/null || true`, and a stub `claude` binary earlier on PATH that writes its argv to a capture file and exits 0 (the real `claude` is what `pm_core.claude_launcher` invokes via `shutil.which("claude")`, so a PATH stub is the legitimate boundary for observing what pm hands to claude).
WHEN: The user runs `pm qa regression echo-smoke` (no flags) from `$TEST_DIR`.
THEN:
- The pm process exits 0 and prints `Running regression: Echo Smoke` followed by the session label.
- The capture file contains the regression body text (the trivial echo instruction).
- The capture file does NOT contain any `--plan bugs` / `--plan improvements` filing-addendum text.

GIVEN: Same setup as the previous triple (pm session running, echo-smoke regression file present, stub `claude` on PATH; clear the capture file before this run).
WHEN: The user runs `pm qa regression echo-smoke --file-prs` from `$TEST_DIR`.
THEN: The capture file contains a filing-findings section that mentions filing both `pm pr add ... --plan bugs ...` and `pm pr add ... --plan improvements ...` commands.

GIVEN: Same setup as above (clear capture file before this run).
WHEN: The user runs `pm qa regression echo-smoke --file-bugs` from `$TEST_DIR` (the hidden legacy alias).
THEN: The capture file contains the same filing-findings text as the `--file-prs` run (the alias maps onto the same `file_prs=True` flag).

GIVEN: The pm install from the setup above (no other state required).
WHEN: The user runs `pm tui test --list`.
THEN: pm exits non-zero with click's `Error: No such command 'test'.` message (the `test` subcommand under `pm tui` has been removed); no regression or TUI action runs.
...
under `pm/qa/captures/pr-6be8ee6/scenarios/48/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/48/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/48/`
- `git commit -m "qa: capture for scenario 48"`
