You are running QA scenario 57: "End-to-end bug-fix session produces pre-fix + post-fix captures and downstream QA scenarios save under the new layout"
...
**Focus**: integration between the new 5-step bug-fix flow, the captures directory layout, and the QA scenario runner

**Steps**:
GIVEN: a throwaway pm test project with the editable clone activated and a freshly-created PR with `plan: bugs` describing a deterministic defect.
- `python3 -m venv /tmp/pm-venv && source /tmp/pm-venv/bin/activate && pip install -e .` from `/workspace`
- `export PYTHONPATH=/workspace` and confirm `pm which` prints `/workspace`, not `/opt/pm-src`
- `TEST_DIR=<workdir>/pm-test-$(date +%s); mkdir -p "$TEST_DIR" && cd "$TEST_DIR" && git init`
- `pm init --backend local --no-import`
- Commit a small helper module with a deterministic bug (e.g. `helper.py` with an off-by-one in `last_index(xs)`) plus a placeholder unit test file, all on the initial commit
- `pm pr add "Fix off-by-one in last_index"`, capture the printed `pr-XXXXXXX` id, then edit `pm/project.yaml` only to set that PR's `plan: bugs` (bootstrap-only edit, per instruction file)
- `cd "$TEST_DIR" && pm session 2>/dev/null || true` (attach failure is expected — session is still created)
WHEN: the test agent renders the bug-fix impl prompt with `pm prompt <pr-id>` and then literally performs the 5 steps it describes — (1) reproduce on pre-fix code and save a capture under `pm/qa/captures/<pr-id>/impl/pre-fix/` (using the `cli-recording.md` recipe in `pm/qa/artifacts/`, with a `manifest.md`), (2) write a failing unit test against the helper, (3) commit the one-line fix, (4) run the test and confirm it passes, (5) re-run the repro on the fixed code and save a capture + `manifest.md` under `pm/qa/captures/<pr-id>/impl/post-fix/`.
THEN:
- `pm/qa/captures/<pr-id>/impl/pre-fix/` exists with `manifest.md` + at least one capture artifact (asciinema cast or transcript) demonstrating the bug
- `pm/qa/captures/<pr-id>/impl/post-fix/` exists with `manifest.md` + capture artifact demonstrating the fixed behavior
- The committed test fails when run against the pre-fix commit (`git stash` the fix or check out the parent and re-run) and passes against the fix commit

GIVEN: the same throwaway project and PR from the prior triple, now with the bug fix committed and pre-fix/post-fix captures present under `pm/qa/captures/<pr-id>/impl/`.
WHEN: from a new pane inside the test tmux session the test agent launches the TUI (`pm tui`), focuses the PR, and triggers the QA-loop start key (the `qa_loop_ui.fresh_start_qa` surface at `pm_core/tui/pr_view.py:469-470` / `pm_core/tui/app.py:1280-1284`) and waits for the loop to plan and execute at least one minimal scenario.
THEN: per-scenario captures land under `pm/qa/captures/<pr-id>/scenarios/<n>/` containing `manifest.md` plus the recording/transcript artifacts called for by the scenario's bound `pm/qa/artifacts/tmux-screen-recording.md` or `cli-recording.md` recipe (matching the existing layout under `pm/qa/captures/pr-6be8ee6/scenarios/1/`: `manifest.md` + `recording.cast` for cli-recording, or `manifest.md` + `screens/` + `transcript.log` + `cmd-pane.log` for tmux-screen-recording).
...
under `pm/qa/captures/pr-6be8ee6/scenarios/57/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/57/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/57/`
- `git commit -m "qa: capture for scenario 57"`
