You are running QA scenario 47: "pm qa docs from outside the source tree (packaging)"
...
**Focus**: The packaged qa_library.md is bundled with the wheel and pm qa docs works from a cwd that has no

**Steps**:
GIVEN: The pm repo is at /workspace. The user runs `./install.sh --local` from /workspace, which builds a venv at `~/.local/share/pm/venv`, installs pm into it (pulling in the packaged `pm_core/docs/qa_library.md` via `pyproject.toml`'s `pm_core = ["docs/*.md"]` data-files entry), and symlinks `~/.local/bin/pm` into it. The user verifies `~/.local/bin` is on PATH and that `which pm` resolves to the symlink (not the editable /workspace checkout). Importantly, the user unsets `PYTHONPATH` for this scenario so the installed wheel is what's actually exercised (otherwise the dev source-tree shadows the install and the packaging path isn't tested).
WHEN: The user `cd /tmp/empty-dir` (a freshly-created directory containing no `pm/` subtree, no `pyproject.toml`, no git repo) and runs `pm qa docs`.
THEN:
- Exit code is 0.
- stdout contains the rendered contents of `pm_core/docs/qa_library.md`, recognizable by the header `# pm QA library` and prose mentioning the `pm/qa/instructions/`, `pm/qa/regression/`, and `pm/qa/artifacts/` subdirectories.
- stderr is empty (or at least free of any `FileNotFoundError`, `No such file or directory`, or Python traceback referencing `qa_library.md`).
...
under `pm/qa/captures/pr-6be8ee6/scenarios/47/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/47/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/47/`
- `git commit -m "qa: capture for scenario 47"`
