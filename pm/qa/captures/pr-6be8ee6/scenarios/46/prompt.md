You are running QA scenario 46: "pm qa CLI — add/author/list/show/edit/docs across three categories"
...
**Focus**: All add-, author-, list, show, edit, and docs commands on the new pm qa surface, including

**Steps**:
GIVEN: A throwaway pm project is initialized in a temp directory with `pm` installed in an isolated venv. Setup:
- `python3 -m venv /tmp/pm-venv && source /tmp/pm-venv/bin/activate`
- `pip install -e /workspace` (the editable clone)
- `export PYTHONPATH=/workspace` so the editable install shadows `/opt/pm-src` (verify with `pm which`)
- `TEST_DIR=$(mktemp -d) && cd "$TEST_DIR" && git init`
- `pm init --backend local --no-import`
- `export EDITOR=true` so `pm qa add-*` returns immediately after scaffolding.
- No files exist yet under `pm/qa/instructions/`, `pm/qa/regression/`, or `pm/qa/artifacts/`.
WHEN: The user runs, from `$TEST_DIR`, in turn:
- `pm qa add-instruction sample-inst`
- `pm qa add-regression sample-reg`
- `pm qa add-artifact sample-art`
THEN:
- Each command exits 0 and prints `Created: <path>`.
- The three files exist at `pm/qa/instructions/sample-inst.md`, `pm/qa/regression/sample-reg.md`, `pm/qa/artifacts/sample-art.md`.
- Each file begins with a `---`-delimited YAML frontmatter containing `title:` and `description:` keys, and contains no `tags:` key (the three add-* templates omit tags; only the `qa mocks add` template includes tags).

GIVEN: The three files created by the previous triple exist.
WHEN: The user runs `pm qa add-instruction sample-inst` again.
THEN: The command exits non-zero, prints an `Already exists: pm/qa/instructions/sample-inst.md` message to stderr, and the pre-existing file's contents are unchanged (compare mtime or sha before/after).

GIVEN: The three files exist from the first triple.
WHEN: The user runs `pm qa list`.
THEN: Output contains three labeled section headers — `Instructions (1):`, `Regression Tests (1):`, `Artifact Recipes (1):` — and lists `sample-inst`, `sample-reg`, `sample-art` under their respective sections (each line formatted as `  <id>: <title>`).

GIVEN: The artifact file `sample-art` exists.
WHEN: The user runs `pm qa show sample-art` (no `-c` flag).
THEN: Exit 0; stdout begins with `# Sample Art` (the title), then a `[<path-to-sample-art.md>]` line, then the body of `pm/qa/artifacts/sample-art.md` — confirming auto-detection found the item without an explicit category hint.

GIVEN: The artifact file `sample-art` exists and `EDITOR=true` is exported.
WHEN: The user runs `pm qa edit sample-art`.
THEN: The command exits 0 with no error output (it resolves the file across categories and runs `$EDITOR` on its path; `true` exits 0 immediately).

GIVEN: No file exists at `pm/qa/instructions/draft-one.md`. A shim binary named `claude` is on PATH (ahead of any real install) that writes its final positional argument to a known file and exits 0 — e.g. a script that does `printf '%s' "${!#}" > /tmp/captured-prompt.txt`. The shim is found by `shutil.which("claude")`, which is what `pm_core.claude_launcher.find_claude` uses.
WHEN: The user runs `pm qa author-instruction draft-one`.
THEN:
- The recorded prompt file contains the qa_library reference content (verify by grepping the captured prompt for a distinctive line from `pm_core/docs/qa_library.md`, e.g. a section header that file contains).
- The recorded prompt contains the literal target path string ending in `pm/qa/instructions/draft-one.md`.
- The command exits 0 (the shim returns 0).

GIVEN: After the previous triple, the user creates `pm/qa/instructions/draft-one.md` (e.g. `touch` it, since the shim does not write the file itself; `_qa_author` only checks `target.exists()` before launching).
WHEN: The user re-runs `pm qa author-instruction draft-one`.
THEN: The command exits non-zero before invoking the shim, printing `Already exists: pm/qa/instructions/draft-one.md` to stderr.

GIVEN: The throwaway project from the first triple.
WHEN: The user runs `pm qa add foo` (no such subcommand exists — only `add-instruction`, `add-regression`, `add-artifact`).
THEN: Click prints a usage error mentioning `No such command 'add'` (or equivalent Click "no such command" wording) to stderr and exits non-zero; no file is created under `pm/qa/`.
...
under `pm/qa/captures/pr-6be8ee6/scenarios/46/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/46/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/46/`
- `git commit -m "qa: capture for scenario 46"`
