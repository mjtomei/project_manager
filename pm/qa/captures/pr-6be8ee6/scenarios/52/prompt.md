You are running QA scenario 52: "Bug-fix flow prompt structure varies by PR plan"
...
**Focus**: _BUG_FIX_FLOW_BLOCK appearance and content for bug PRs vs non-bug PRs

**Steps**:
GIVEN: a throwaway pm project with pm installed in an isolated venv, containing a PR whose plan is "bugs":
- Install pm into a fresh venv from the project_manager clone:
```
python3 -m venv /tmp/pm-venv && source /tmp/pm-venv/bin/activate
pip install -e /workspace
export PYTHONPATH=/workspace   # override container's /opt/pm-src master shadow
pm which  # confirm it points at /workspace, not /opt/pm-src
```
- Create a throwaway project under your workdir (fallback `/tmp`) and `git init` it.
- Inside that dir, run `pm init --backend local --no-import`, then create a plan named "bugs" and add two PRs — one in the "bugs" plan and one in a non-bug plan (e.g. "features"):
```
pm plan add bugs
pm plan add features
pm pr add "Crash on empty input" --plan bugs
pm pr add "Add dark mode" --plan features
```
Note the auto-generated PR ids (`pr-XXXXXXX`) printed by each `pm pr add`; refer to them as `<BUG_PR>` and `<FEAT_PR>` below.

WHEN: the user renders the implementation session prompt for the bug PR via `pm prompt <BUG_PR>`.

THEN:
- stdout contains a section header `## Bug Fix Flow`.
- That section contains exactly five numbered items in this order, with these bold headings:
1. `**Manual repro on pre-fix code**`
2. `**Write a failing test**`
3. `**Fix**`
4. `**Verify with the test**`
5. `**Verify manually**`
- The block references both `pm/qa/instructions/` (env-setup recipes) and `pm/qa/artifacts/` (capture recipes).
- The block references `pm/qa/captures/<BUG_PR>/impl/pre-fix/` and `pm/qa/captures/<BUG_PR>/impl/post-fix/`, with the actual bug PR's id interpolated in place of `<BUG_PR>`.
- Step 1's body mentions reusing a valid prior capture under `pm/qa/captures/<BUG_PR>/impl/pre-fix/` and falling back to stashing uncommitted changes or checking out the parent commit / reverting fix files when the fix is already committed.

GIVEN: the same throwaway project (bug PR plus non-bug PR).

WHEN: the user renders the review prompt for the same bug PR. There is no dedicated `--print-prompt` flag for review prompts, so drive it through the actual review surface: from inside the test tmux session, run `pm pr review <BUG_PR>` in a new pane, then capture that pane's scrollback with `tmux capture-pane -p -t <session>:<window>.<pane> -S -` to recover the prompt text passed to `claude`.

THEN: the captured prompt contains a `## Bug Fix Review Checklist` section, and the bullet about pre-fix/post-fix captures under `pm/qa/captures/<BUG_PR>/impl/` says missing or unconvincing captures should be surfaced as `INPUT_REQUIRED` (not `NEEDS_WORK`).

GIVEN: the same project, focusing on the non-bug (`features`-plan) PR.

WHEN: the user runs `pm prompt <FEAT_PR>`.

THEN: stdout has no `## Bug Fix Flow` section anywhere.
...
under `pm/qa/captures/pr-6be8ee6/scenarios/52/` (each
...
`pm/qa/captures/pr-6be8ee6/scenarios/52/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/52/`
- `git commit -m "qa: capture for scenario 52"`
