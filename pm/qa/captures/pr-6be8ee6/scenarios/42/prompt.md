You are running QA scenario 42: "Packaging — pm qa docs works after pip install from any cwd"
...
**Focus**: pm_core/docs/qa_library.md is shipped as package data and pm qa docs resolves it regardless of cwd

**Steps**:
1. Confirm the source tree at /workspace declares `qa_library.md` as package data: `grep -A1 'package-data' /workspace/pyproject.toml` should show `pm_core = ["docs/*.md"]`, and `ls /workspace/pm_core/docs/qa_library.md` should exist. These are the artifacts under test.

2. The container sets `PYTHONPATH=/opt/pm-src`, which would shadow any installed copy with the master source. Unset it for this packaging test so we are exercising the real installed package:
```
unset PYTHONPATH
```

3. Install pm from the /workspace clone into a fresh isolated venv via the project's installer:
```
cd /workspace
./install.sh --local --force
```
This creates `~/.local/share/pm/venv` and symlinks `~/.local/bin/pm`. Confirm the install with:
```
ls -l ~/.local/bin/pm
~/.local/bin/pm which
```
`pm which` must print a path under `~/.local/share/pm/venv/...` (NOT `/workspace` and NOT `/opt/pm-src`) — that proves we are running the packaged install, not an editable shadow.

4. cd to a directory with no pm project, then invoke `pm qa docs` via the installed entry point:
```
cd /tmp
~/.local/bin/pm qa docs > /tmp/qa-docs.out
echo "exit=$?"
```
Confirm exit code is 0 and `/tmp/qa-docs.out` is non-empty.

5. Verify the output contains substantive content from `qa_library.md` — check for the actual section headers that exist in the source file (`## Instructions`, `## Regression tests`, `## Artifact recipes`, plus the top-level `# pm QA library` title):
```
...
```
Each should be ≥1. Note that the headers are lowercased after the first word ("Regression tests", "Artifact recipes") — do not match "Regression Tests" capitalized.

6. Confirm non-trivial output size from the same cwd:
```
cd /tmp
~/.local/bin/pm qa docs | wc -l
```
Expect > 50 lines (the source file is ~342 lines; output should be similar).

7. Compare bundled output to source-of-truth to detect any truncation or wrong file resolution:
```
diff <(~/.local/bin/pm qa docs) /workspace/pm_core/docs/qa_library.md
```
Expect no diff (or only a trailing-newline diff). This proves the resolver located the package-data file from inside the installed wheel, not a stale copy.

8. Create a fresh pm-initialized throwaway project and re-run from inside it to confirm no project-specific override path is consulted and no missing-file error appears:
```
TEST_DIR=/tmp/pm-pkg-test-$(date +%s)
mkdir -p "$TEST_DIR" && cd "$TEST_DIR"
git init -q
~/.local/bin/pm init --backend local --no-import
~/.local/bin/pm qa docs > /tmp/qa-docs-proj.out
echo "exit=$?"
diff /tmp/qa-docs.out /tmp/qa-docs-proj.out
```
Expect exit 0, no missing-file error on stderr, and identical output to the /tmp run (no project override).

9. Sanity-check that the resolver is not accidentally reading from the /workspace source tree (which would mask a real packaging bug). From a cwd unrelated to /workspace, with PYTHONPATH unset, run:
```
cd /tmp
~/.local/share/pm/venv/bin/python -c "import pm_core, importlib.resources, pathlib; print(pathlib.Path(pm_core.__file__).resolve())"
```
The printed pm_core path must be under `~/.local/share/pm/venv/`, confirming the package data was loaded from the installed wheel.

10. Capture the CLI session evidence under the scenario's capture directory:
```
mkdir -p pm/qa/captures/scenarios/42
cp /tmp/qa-docs.out pm/qa/captures/scenarios/42/qa-docs-tmp.out
cp /tmp/qa-docs-proj.out pm/qa/captures/scenarios/42/qa-docs-project.out
~/.local/bin/pm which > pm/qa/captures/scenarios/42/pm-which.txt
```
(Run from inside `$TEST_DIR` so captures land under the throwaway project's `pm/qa/captures/`.) Record the exit codes, line counts, and diff results in the scenario verdict.
...
`pm/qa/captures/pr-6be8ee6/scenarios/42/` (each recipe's
...
`pm/qa/captures/pr-6be8ee6/scenarios/42/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/42/`
- `git commit -m "qa: capture for scenario 42"`
