You are running QA scenario 14: "qa-finalize wiring in run_qa_sync"
...
**Focus**: build_qa_finalize_prompt content, _run_qa_finalize_pane None-paths, and state.finalize_verdict + latest_output [finalize: …] suffix population

**Steps**:
1. Verify the code under test exists and read the relevant sections:
- `grep -n "finalize_verdict\|_run_qa_finalize_pane\|finalize_suffix" /workspace/pm_core/qa_loop.py` — confirm lines ~172 (default None), ~364 (def `_run_qa_finalize_pane`), ~2908-2927 (wiring + `[finalize: …]` suffix) are present.
- `grep -n "def build_qa_finalize_prompt\|FINALIZE_DONE\|FINALIZE_BLOCKED" /workspace/pm_core/qa_finalize_prompt.py` — confirm the builder and the two literal verdict tokens.

2. Create `/workspace/tests/test_qa_finalize_wiring.py` with focused unit tests (do not attempt to drive full `run_qa_sync` — it requires a live tmux pm session, store, planner pane, scenario-launch and polling subsystems; covering those with monkey-patches is out of scope and would make the test brittle). Tests to include:

a. **`test_finalize_verdict_default_none`** — instantiate `QALoopState(pr_id="pr-test")` and assert `state.finalize_verdict is None`.

b. **`test_build_qa_finalize_prompt_contains_all_fields`** — call `build_qa_finalize_prompt(pr_id="pr-abc", pr_title="Some title", branch="feat/x", pr_workdir="/tmp/wd", scenario_worktrees=[(1, "PASS", "/tmp/w1"), (2, "NEEDS_WORK", "/tmp/w2")], overall_verdict="PASS")` (note: the real signature also requires `pr_title` — the planner's step 4 omitted it). Assert the returned string contains: `"pr-abc"`, `"Some title"`, `"feat/x"`, `"/tmp/wd"`, `"PASS"`, `"Scenario 1"`, `"/tmp/w1"`, `"Scenario 2"`, `"NEEDS_WORK"`, `"/tmp/w2"`. Also assert that `"\nFINALIZE_DONE"` and `"\nFINALIZE_BLOCKED"` appear (each starts a new line — they are inside list items prefixed with `` ` ``, so check for both `"FINALIZE_DONE"` and `"FINALIZE_BLOCKED"` substrings AND that each begins a list item via the regex `r"^- `FINALIZE_DONE`"` and `r"^- `FINALIZE_BLOCKED`"` with `re.MULTILINE`).

c. **`test_build_qa_finalize_prompt_empty_scenarios`** — call with `scenario_worktrees=[]`; assert `"(no scenarios ran)"` appears.

d. **`test_build_qa_finalize_prompt_none_fields_render`** — call with `scenario_worktrees=[(3, None, None)]`; assert `"verdict=?"` and `"worktree=(none)"` appear.

e. **`test_run_qa_finalize_pane_returns_none_when_workdir_missing`** — call `_run_qa_finalize_pane(state=QALoopState(pr_id="x"), pr_data={}, session="pm-test", window_name="qa", workdir_path=None)` and assert it returns `None`. (No monkeypatch needed — early return at `qa_loop.py:382`.)

f. **`test_run_qa_finalize_pane_returns_none_when_window_missing`** — monkeypatch `pm_core.tmux.find_window_by_name` to return `None`. Build a `QALoopState(pr_id="x")` with empty `scenario_verdicts` and `verified_scenarios` (so the unverified gate passes), call `_run_qa_finalize_pane(state, {}, "pm-test", "qa-window", "/tmp")` with a real existing dir (use `tmp_path`). Assert returns `None`.

g. **`test_run_qa_finalize_pane_returns_none_when_pass_unverified`** — build a state with `state.scenario_verdicts = {1: "PASS"}` and `state.verified_scenarios = set()`. Call with a real `tmp_path` workdir; assert returns `None` (gate at lines 388-395). No tmux patch needed since the gate fires before tmux access.

h. **`test_run_qa_sync_finalize_suffix_in_latest_output`** (the wiring test the planner asked for, made tractable): rather than driving the entire `run_qa_sync`, patch the *whole* function pipeline minimally by:
- monkeypatching `pm_core.qa_loop.get_pm_session` → `"pm-test"`,
- `pm_core.store.load` → `{"prs": [{"id": "pr-x", "branch": "b", "title": "t", "workdir": str(tmp_path)}]}` (use `tmp_path` so the workdir check passes),
- `pm_core.qa_loop._run_qa_finalize_pane` → stub returning `"FINALIZE_DONE"`,
- `pm_core.qa_loop._launch_scenarios_in_tmux` → no-op,
- `pm_core.qa_loop._launch_scenarios_in_containers` → no-op,
- `pm_core.qa_loop._poll_tmux_verdicts` → no-op,
- `pm_core.qa_loop._persist_scenario_verdicts` → no-op,
- `pm_core.qa_loop._write_status_file` → no-op,
- `pm_core.tmux.find_window_by_name` → return `{"index": "0", "id": "@0"}`,
- `pm_core.tmux.get_pane_indices` → return `[("%0", "0")]`,
- `pm_core.qa_loop.create_qa_workdir` → return `tmp_path / "qa"` (ensure mkdir).
- Pre-populate `state.scenarios = [QAScenario(index=1, title="t", focus="f")]`, `state.scenario_verdicts = {1: "PASS"}`, `state.verified_scenarios = {1}`, `state.planning_phase = False` (skip planning).
- Pre-populate a fake QA spec on the PR entry (or monkeypatch `_get_qa_spec` to return truthy) so the spec gate at line 2745 does not return early.
- Call `run_qa_sync(state, tmp_path, pr_data)`. Assert `state.finalize_verdict == "FINALIZE_DONE"` and `"[finalize: FINALIZE_DONE]"` is in `state.latest_output`.
- **If** the cascade of required monkeypatches exceeds ~12 helpers or hits a tmux/subprocess call that cannot be cleanly patched, drop this sub-test and instead assert the wiring textually: read `qa_loop.py`, slice lines 2908-2927, and assert the slice contains both `state.finalize_verdict = _run_qa_finalize_pane(` and `f" [finalize: {state.finalize_verdict}]"`. Document the trade-off in a `# NOTE:` comment.

i. **`test_run_qa_sync_swallows_finalize_exception`** — same harness as (h) but stub `_run_qa_finalize_pane` to raise `RuntimeError("boom")`. Assert `run_qa_sync` returns normally, `state.finalize_verdict is None`, and `state.latest_output` does NOT contain `"[finalize:"` (suffix only added when verdict truthy — lines 2921-2923). If the harness in (h) was downgraded to a textual assertion, downgrade this one the same way: assert the `try/except` block at lines 2903-2914 wraps the call and logs via `_log.exception`.

3. Run the tests and save logs:
- `mkdir -p /workspace/scenarios/14`
- Record the run with asciinema if available: `asciinema rec --command "cd /workspace && python -m pytest tests/test_qa_finalize_wiring.py -v 2>&1 | tee /workspace/scenarios/14/pytest.log" /workspace/scenarios/14/run.cast` — if asciinema isn't installed, just run `cd /workspace && python -m pytest tests/test_qa_finalize_wiring.py -v 2>&1 | tee /workspace/scenarios/14/pytest.log` and skip the cast.
- Confirm exit code is 0 and all created tests passed; save a short `/workspace/scenarios/14/manifest.md` noting which sub-tests of step 2(h)/(i) were full integration vs textual-assertion fallback.

4. Verdict criteria:
- PASS — `finalize_verdict` default is None, builder includes every input field plus both verdict tokens on their own list lines, all three `_run_qa_finalize_pane` None-paths (no workdir / no window / unverified PASS) return None, and the suffix wiring exists in `run_qa_sync` (either verified by integration test or by textual assertion).
- NEEDS_WORK — any of the above fails or the builder is missing a field/token, or `run_qa_sync` does not assign `finalize_verdict` / does not include the `[finalize: …]` suffix.
...
`pm/qa/captures/pr-6be8ee6/scenarios/14/` (each recipe's
...
`pm/qa/captures/pr-6be8ee6/scenarios/14/pre-fix/` and the
...
PR branch:
- `git add pm/qa/captures/pr-6be8ee6/scenarios/14/`
- `git commit -m "qa: capture for scenario 14"`
