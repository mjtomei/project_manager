...
def _find_impl_pane(session: str, window_name: str) -> str | None:
    """Find the Claude pane in an implementation or merge window.

    Delegates to :func:`pm_core.loop_shared.find_claude_pane` which
    checks the pane registry first (any ``*-claude`` role), then falls
    back to ``panes[0][0]`` for legacy/unregistered windows.
    """
