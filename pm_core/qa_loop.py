...
_CONCRETIZE_VERDICTS = ("STEPS_VERIFIED",)
_CONCRETIZE_KEYWORDS = ("STEPS_VERIFIED",)
...
The steps above were generated by a planner that may have hallucinated
...
1. Run `git diff {base_branch}...{pr_branch}` to see what changed in this PR
2. For each planned step, check:
   - Do the referenced files, functions, and commands actually exist?
   - Are the command-line flags and arguments correct?
   - Are the expected outputs realistic based on the actual code?
3. Rewrite any steps that reference non-existent things
4. Keep steps that are already correct unchanged
5. If a step is fundamentally impossible in this environment, note why
