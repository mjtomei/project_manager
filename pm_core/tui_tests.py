...
3. Record initial state:
   ...
   - `cat ~/.pm/pane-registry/<session>.json` — note registered panes and roles
4. Run `pm tui view` to verify TUI is running
...
4. Check pane registry:
   - `cat ~/.pm/pane-registry/<session>.json`
   - Look for a pane with role "meta" or related
...
2. Alternatively, to clean up:
   ...
   - Check registry was updated: `cat ~/.pm/pane-registry/<session>.json`
...
3. Record initial state:
   - `tmux list-panes -t <session> -F "#{pane_id} #{pane_current_command}"` — count panes
   - `cat ~/.pm/pane-registry/<session>.json` — note registered panes and roles
   - Check if a log pane already exists (role "log")
4. Run `pm tui view` to verify TUI is running
...
3. Check pane registry:
   - `cat ~/.pm/pane-registry/<session>.json`
   - Should have a pane with role "log"
...
3. Check registry:
   - `cat ~/.pm/pane-registry/<session>.json`
   - Should still have exactly ONE pane with role "log"
4. Check log line:
   - `pm tui view`
...
3. Verify cleanup:
   - `cat ~/.pm/pane-registry/<session>.json`
   - The log pane should be unregistered (role "log" gone)
...
5. Verify relaunch:
   ...
   - `cat ~/.pm/pane-registry/<session>.json` — role "log" should be back
