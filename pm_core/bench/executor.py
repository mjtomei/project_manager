...
_LANG_CONFIG: dict[str, dict] = {
    ...
    "cpp": {
        ...
        "test_cmd": ["bash", "-c", "cmake -B build . && cmake --build build && cd build && ctest --output-on-failure"],
    },
    "java": {
...
def execute_tests(
    ...
    *,
    ...
    Args:
        exercise: The exercise (provides language, slug, and exercise_dir).
        candidate_code: The candidate's source code.
        test_code: Custom test code to use instead of the exercise's reference tests.
                   If None, uses whatever tests are already in the exercise dir.
    ...
    with tempfile.TemporaryDirectory(prefix=f"pm-bench-{lang}-") as tmp:
        ...
        shutil.copytree(exercise.exercise_dir, work_dir, dirs_exist_ok=True)
        ...
        if solution_file is None:
            ...
            if java_files:
                solution_path = java_files[0]
            else:
                return ScoreResult(error="Cannot find Java source file")
        else:
            solution_path = work_dir / solution_file
...
def _parse_test_output(output: str, returncode: int, language: str) -> ScoreResult:
    ...
    if returncode == 0:
        # All tests passed — try to count them
    ...
    # Some tests failed — try to parse counts
    ...
    if total == 0:
        # Couldn't parse output — probably a compilation error
...
def _count_tests_from_output(output: str, language: str) -> int:
    ...
    # Default: at least 1 test ran if returncode was 0
    return 1
...
def _parse_counts(output: str, language: str) -> tuple[int, int]:
    ...
    # pytest: "2 passed, 3 failed" or "5 passed"
    passed = 0
    failed = 0
