...
and are cached locally under ~/.cache/pm-bench/.
...
def _parse_exercise(exercise_dir: Path, language: str) -> Exercise | None:
    ...
    for pattern in solution_globs:
        for match in exercise_dir.glob(pattern):
            if match.is_file():
                starter_code[str(match.relative_to(exercise_dir))] = _read_text(match)
    ...
    for pattern in test_globs:
        for match in exercise_dir.glob(pattern):
            if match.is_file():
                reference_tests[str(match.relative_to(exercise_dir))] = _read_text(match)
...
def _find_test_files(exercise_dir: Path, language: str) -> dict[str, str]:
    ...
    for pat in patterns.get(language, []):
        for match in exercise_dir.glob(pat):
            if match.is_file():
                result[str(match.relative_to(exercise_dir))] = _read_text(match)
    return result
