...
from pm_core.bench.runner import CostMetrics, OllamaRunner
...
class ExerciseResult:
    ...
    # Tournament path: gen tests → gen N candidates → score → pick best → reference score
    tournament_score: float = 0.0
    tournament_best_gen_score: float = 0.0  # score against generated tests
...
def run_exercise_tournament(
    ...
    *,
    ...
    try:
        ...
        # Step 4: Pick the best candidate (highest score on generated tests)
        ...
        result.error = f"Ollama connection error: {exc}"
...
def run_benchmark(
    ...
    *,
    ...
    Args:
        ...
        cache_dir: Override exercise cache dir.
        progress_callback: Called with (exercise, status_message) updates.
    """
    runner = OllamaRunner()
    ...
    exercises = load_exercises(
        languages=languages, slugs=slugs, **load_kwargs
    )
...
def format_results_table(run: BenchmarkRun) -> str:
    ...
    # Column headers
    header = f"{'Language':<12} {'Exercise':<30} {'Tournament':>10} {'Baseline':>10} {'Delta':>8} {'Tokens':>8}"
    lines.append(header)
    lines.append("-" * 80)
