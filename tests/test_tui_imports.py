...
class TestWidgetImports:
    ...
    def test_status_bar_has_update_status(self):
        ...
        assert "memory_status" in params
...
# ---------------------------------------------------------------------------
# StatusBar.update_status memory_status markup integration
# ---------------------------------------------------------------------------

class TestStatusBarMemoryMarkup:
    def test_memory_status_renders_bold_cyan(self):
        from unittest.mock import patch, MagicMock
        from pm_core.tui.widgets import StatusBar
        bar = StatusBar()
        with patch.object(bar, "update") as mock_update:
            bar.update_status("proj", "repo", "synced",
                              memory_status="34G/48G (pm)")
            args = mock_update.call_args
            markup = args[0][0] if args[0] else args[1].get("content", "")
            assert "[bold cyan]34G/48G (pm)[/bold cyan]" in markup

    def test_empty_memory_status_no_bold_cyan(self):
        from unittest.mock import patch
        from pm_core.tui.widgets import StatusBar
        bar = StatusBar()
        with patch.object(bar, "update") as mock_update:
            bar.update_status("proj", "repo", "synced", memory_status="")
            args = mock_update.call_args
            markup = args[0][0] if args[0] else args[1].get("content", "")
            assert "[bold cyan]" not in markup


# ---------------------------------------------------------------------------
# app.py _update_status_bar memory_status integration
# ---------------------------------------------------------------------------

class TestAppMemoryStatusIntegration:
    def test_format_memory_status_passed_to_update_status(self):
        """Verify _update_status_bar passes format_memory_status result."""
        from unittest.mock import patch, MagicMock
        from pm_core.tui.app import ProjectManagerApp

        app = ProjectManagerApp.__new__(ProjectManagerApp)
        app._data = {"project": {"name": "test", "repo": "r"}, "prs": []}
        app._auto_start = False

        mock_tree = MagicMock()
        mock_tree._status_filter = ""
        mock_tree._hide_merged = False
        mock_tree._hide_closed = False
        mock_tree._sort_field = ""
        mock_status_bar = MagicMock()
        mock_session_bar = MagicMock()
        mock_watcher = MagicMock()
        mock_watcher.is_any_running.return_value = False
        app._watcher_manager = mock_watcher

        def query_one_side_effect(selector, cls=None):
            lookup = {
                "#tech-tree": mock_tree,
                "#status-bar": mock_status_bar,
                "#session-bar": mock_session_bar,
            }
            return lookup[selector]

        app.query_one = query_one_side_effect

        with patch("pm_core.memory_governor.format_memory_status",
                   return_value="34G/48G (pm)"), \
             patch("pm_core.paths.get_global_setting", return_value=False):
            app._update_status_bar()

        mock_status_bar.update_status.assert_called_once()
        kwargs = mock_status_bar.update_status.call_args[1]
        assert kwargs.get("memory_status") == "34G/48G (pm)"

    def test_format_memory_status_exception_fallback(self):
        """Verify update_status gets memory_status='' on exception."""
        from unittest.mock import patch, MagicMock
        from pm_core.tui.app import ProjectManagerApp
        ...
        mock_status_bar.update_status.assert_called_once()
        kwargs = mock_status_bar.update_status.call_args[1]
        assert kwargs.get("memory_status") == ""
