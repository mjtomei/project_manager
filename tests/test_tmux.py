...
class TestFocusWindow:
    ...
    def test_default_switches_only_origin(self, mock_fwbn, mock_run):
        """Default behaviour: only the originating session switches.

        Sessions that happen to be on the same window are not dragged
        along — that would be wrong for the multi-attached single-TUI
        case.
        """
        ...
        assert "proj~2:3" in " ".join(sw_calls[0].args[0])

    @patch("pm_core.tmux.subprocess.run")
    @patch("pm_core.tmux.find_window_by_name",
           return_value={"id": "@9", "index": "3", "name": "review-x"})
    def test_explicit_origin_does_not_query_env(self, mock_fwbn, mock_run):
        """With an explicit origin_session, env/legacy fallbacks are skipped."""
        mock_run.return_value = MagicMock(returncode=0, stdout="")
        with patch("pm_core.tmux.current_or_base_session") as mock_cobs:
            focus_window("proj", "review-x", origin_session="proj~2")
            mock_cobs.assert_not_called()

    @patch("pm_core.tmux.subprocess.run")
    @patch("pm_core.tmux.find_window_by_name",
           return_value={"id": "@9", "index": "3", "name": "review-x"})
    def test_picks_origin_from_env(self, mock_fwbn, mock_run):
        mock_run.return_value = MagicMock(returncode=0, stdout="")
        ...
        sw_calls = [c for c in mock_run.call_args_list
                    if "select-window" in c.args[0]]
        assert any("proj~5:3" in " ".join(c.args[0]) for c in sw_calls)

    @patch("pm_core.tmux.subprocess.run")
    @patch("pm_core.tmux.find_window_by_name",
           return_value={"id": "@9", "index": "3", "name": "review-x"})
    def test_co_viewers_override_replaces_origin(self, mock_fwbn, mock_run):
        """Pre-captured co_viewers list (loop kill+recreate) replaces origin."""
        mock_run.return_value = MagicMock(returncode=0, stdout="")
        ...
        sw_calls = [c for c in mock_run.call_args_list
                    if "select-window" in c.args[0]]
        targets = [" ".join(c.args[0]) for c in sw_calls]
        assert any("proj:3" in t for t in targets)
        assert any("proj~1:3" in t for t in targets)
