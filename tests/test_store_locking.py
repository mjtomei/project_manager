...
class TestFilePermissions:
    """Tests for read-only permissions and YAML header."""

    def test_save_sets_read_only(self, project_dir):
        """After save, project.yaml should be read-only."""
        import stat
        ...
        assert not (mode & stat.S_IWUSR), f"File is writable: {oct(mode)}"
        # Owner should have read permission
        assert mode & stat.S_IRUSR

    def test_locked_update_can_write_read_only_file(self, project_dir):
        """locked_update should handle a read-only file from a previous save."""
        # First save makes it read-only
        store.locked_update(
            project_dir,
            lambda d: d["prs"].append({"id": "pr-001", "title": "First", "status": "pending"}),
        )
        # Second save should succeed despite read-only
        store.locked_update(
            project_dir,
            lambda d: d["prs"].append({"id": "pr-002", "title": "Second", "status": "pending"}),
        )
        data = store.load(project_dir)
        assert len(data["prs"]) == 2

    def test_yaml_header_present(self, project_dir):
        """Saved file should contain the advisory header."""
        store.locked_update(project_dir, lambda d: None)
        path = project_dir / "project.yaml"
        content = path.read_text()
        assert "Do not edit this file directly" in content
        assert "pm edit" in content

    def test_header_does_not_corrupt_data(self, project_dir):
        """Header comments should not affect loaded data."""
        store.locked_update(
            project_dir,
            lambda d: d["prs"].append({"id": "pr-001", "title": "Test", "status": "pending"}),
        )
        data = store.load(project_dir)
        assert data["prs"][0]["id"] == "pr-001"
        assert "WARNING" not in str(data)  # Comments should not leak into data

    def test_locked_edit_with_noop_editor(self, project_dir):
        """locked_edit with a no-op editor should round-trip cleanly."""
        import stat
        ...
        # Use 'true' as editor (exits immediately, no changes)
        os.environ["EDITOR"] = "true"
        try:
            data = store.locked_edit(project_dir)
        finally:
            os.environ.pop("EDITOR", None)

        assert data["prs"][0]["id"] == "pr-001"
        # Should be read-only after
        path = project_dir / "project.yaml"
        mode = path.stat().st_mode
